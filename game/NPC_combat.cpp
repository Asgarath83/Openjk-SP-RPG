...
extern qboolean meleeWeap(int wp);
	...
	{//delay their attack based on how far away they're facing from enemy
		...
		*/
		...
		case WP_NOGHRI_STICK:
			attDelay += Q_irand( 0, 500 );
		case WP_CANNON:
			attDelay += Q_irand( 500, 1200 );
			break;
		case WP_POISON:
			attDelay += Q_irand( 0, 500 );
			break;
		case WP_JAWA:
			attDelay += Q_irand( 1000, 1200 );
			break;
		/*
		case WP_DEMP2:
			break;
...
////// OLD CHOOSE BEST WEAPON BACK UP 
...
{//this and the funcs it calls do the heavy lifting for weapon grouping
	if ((seekerWeap(weapon) && wpnGroup == WEAPS_SEEKER) // Glifiche
		|| (laserWeap(weapon) && wpnGroup == WEAPS_LASER) // Arrows
		|| (bombsWeap(weapon) && wpnGroup == WEAPS_BOMBS) // nec Holy
		|| (chaffWeap(weapon) && wpnGroup == WEAPS_CHAFF )// Magic  
		|| ( specialWeap(weapon) && wpnGroup == WEAPS_SPECIAL )  // Warrior 
...
//			ent->NPC->burstSpacing = 2500;//attack debounce
		...
		break;
		// Una balestra si usa come un fucile più o m eno...
		case WP_CROSSBOW:
		if ( ent->NPC->scriptFlags & SCF_ALT_FIRE )
		{
			ent->NPC->aiFlags |= NPCAI_BURST_WEAPON;
	...
	case WP_CANNON:
		...
		else
		{
			...
			case 0:
				...
				break;
			case 1:
				ent->NPC->burstSpacing = 1250;//attackdebounce
				break;
			case 2:
				ent->NPC->burstSpacing = 1000;//attackdebounce
				break;
	...
	//	ent->NPC->burstSpacing = 1000;//attackdebounce
		...
		{//rocket
			ent->NPC->burstMin = 1; // 3 shots, really
			ent->NPC->burstMax = 3;
			if ( g_spskill->integer == 0 )
				ent->NPC->burstSpacing = 2500;//attack debounce
			else if ( g_spskill->integer == 1 )
	...
	case WP_DRONE:
		...
		{//rocket
			if ( g_spskill->integer == 0 )
				...
				ent->NPC->burstSpacing = 7500;//attack debounce
			else 
				ent->NPC->burstSpacing = 5000;//attack debounce
		}
		break;
		...
		if ( ent->NPC->scriptFlags & SCF_ALT_FIRE )
		{//beam
			ent->NPC->burstSpacing = 30000;//attackdebounce
		}
		else
		{//rocket
			if ( g_spskill->integer == 0 )
				ent->NPC->burstSpacing = 15000;//attack debounce
			else if ( g_spskill->integer == 1 )
				ent->NPC->burstSpacing = 10000;//attack debounce
			else 
				ent->NPC->burstSpacing = 5000;//attack debounce
		...
		{
			if ( g_spskill->integer == 0 )
				ent->NPC->burstSpacing = 60000;//attack debounce
			else if ( g_spskill->integer == 1 )
				ent->NPC->burstSpacing = 40000;//attack debounce
			else 
				ent->NPC->burstSpacing = 20000;//attack debounce
		}
		else
		{
			if ( g_spskill->integer == 0 )
				ent->NPC->burstSpacing = 30000;//attack debounce
			else if ( g_spskill->integer == 1 )
				ent->NPC->burstSpacing = 20000;//attack debounce
			else 
				ent->NPC->burstSpacing = 10000;//attack debounce
...
void NPC_ChangeWeapon( int newWeapon ) 
{
	...
	if ( changing && NPC->client->ps.weapon != WP_NONE )
	{
		...
		else if ( NPC->client->ps.weapon == WP_BLASTER_PISTOL )
		{
			WP_GunAddG2GunModels( NPC );
		}
		else
		{
...
void NPC_ChangeWeapon(/*gentity_t* NPC,*/ int newWeapon)
{
	...
	if (changing && NPC->client->ps.weapon != WP_NONE)
	{
		if ( changing && NPC->client->ps.weapon == WP_SABER)
		{
			WP_SaberAddG2SaberModels(NPC);
		}
		else if ( changing && NPC->client->ps.weapon == WP_BLASTER_PISTOL ) 
		{
			WP_GunAddG2GunModels ( NPC );
		} 
		else
...
// DUSTY ADD 4 REMOVING ORIGINAL HAVEWEAPON 
