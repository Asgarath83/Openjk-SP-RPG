...
qboolean Jedi_DodgeEvasion( gentity_t *self, gentity_t *shooter, trace_t *tr, int hitLoc )
{
	...
	{//if an NPC, check game skill setting
		// Activated for avoid crash using boss classes againg scouts. 
		// On Deault code this part should be unavailable. 
		if ( self->NPC && (self->NPC->aiFlags&NPCAI_BOSS_CHARACTER) )
	...
	case HL_WAIST:
		...
		else
		{
			...
			{//no fancy dodges when raging or recovering - No Dodge for not grenadier warriors. 
				return qfalse;
			}
	...
	{//check to see if he's coming at me
		...
		{//coming at/facing me!  
			...
			if ( NPC->client->NPC_class == CLASS_SABOTEUR )
			{
				...
				|| NPC->client->ps.saberInFlight 
				|| NPC->client->NPC_class == CLASS_BOBAFETT || NPC->client->NPC_class == CLASS_WIZARD 
			...
			{//hunt him down
				...
				if ( (NPC_ClearLOS( NPC->enemy )||NPCInfo->enemyLastSeenTime>level.time-500) && NPC_FaceEnemy( qtrue ) )//( NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG ) && 
				{
					...
					*/
					...
					{
						Boba_FireDecide();
					}// DUSTY SPECIAL AI CLASS FOR ALLOWS NPC TO SHOOT PLAYER BY LARGE DISTANCE
					else if (NPC->client && NPC->client->NPC_class==CLASS_WIZARD)
					{
						NPC_FireDecide();
					}
...
static void Jedi_Attack( void )
{
	...
	{//check other special combat behavior
		...
		if ( NPC->client->NPC_class == CLASS_ALORA || NPC->client->NPC_class == CLASS_VAMPIRE_WATER )
		{
...
void NPC_BSJedi_Default( void )
{
	...
	{//don't have an enemy, look for one
		if ( NPC->client->NPC_class == CLASS_BOBAFETT || NPC->client->NPC_class == CLASS_WIZARD
			|| (NPC->client->NPC_class == CLASS_REBORN && NPC->s.weapon != WP_SABER)
			|| NPC->client->NPC_class == CLASS_ROCKETTROOPER )
		{
			NPC_BSST_Patrol();
