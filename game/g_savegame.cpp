...
extern void WP_GunSetDefaults ( gunData_t *gun, qboolean setGuns = qtrue ); 
...
static void SG_ConvertRetailGunDataToNewGunData( void *gunData, gunData_t *GunNew )
{
	gunDataRetail_t *GunRetail = ((gunDataRetail_t *)(gunData));
...
{	
	...
	if (iReadSize != iSize)
	{
		...
		switch (ulChid)
		{
			...
			//	AGGIUNGILO DOPO CHE RISOLVI IL CRASH! 
			/*if ( iSize == (int)(iReadSize+((sizeof(gunData_t)-sizeof(gunDataRetail_t))*2)) )
				{
					gclient_t newClient;
					const int	preGunDataSize = ((intptr_t)&newClient.gs.gun[0]-(intptr_t)&newClient);
					memcpy( &newClient, pbData, preGunDataSize );
					SG_ConvertRetailGunDataToNewGunData( ((void *)(&((gclient_t *)(pbData))->gs.gun[0])), &newClient.gs.gun[0] ); 
					memcpy( &newClient.gs.dualGuns, pbData+preGunDataSize+(sizeof(gunDataRetail_t)*2), sizeof(newClient)-(preGunDataSize+(sizeof(gunData_t)*2)) );
					memcpy( pbData, &newClient, sizeof(gclient_t) );
				}
				else
