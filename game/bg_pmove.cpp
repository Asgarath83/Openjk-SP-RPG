...
static void PM_Friction( void ) {
		...
		&& pm->gent->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL )
	{
		...
		else if ( pm->gent->client->NPC_class == CLASS_VEHICLE )
		{// a Fighter has no friction? 
			friction = 0.10f;
		}
	...
	else if ( Flying != FLY_NORMAL )
	{
		...
		pm->gent->client->NPC_class == CLASS_RAZIELHIM || pm->gent->client->NPC_class == CLASS_ESKANDOR ||
		pm->gent->client->NPC_class == CLASS_BIRD ||
		 pm->gent->client->NPC_class == CLASS_POLTER ||
		 pm->gent->client->NPC_class == CLASS_REAPER ||
		 pm->gent->client->NPC_class == CLASS_ARIEL ||
		 pm->gent->client->NPC_class == CLASS_VAMPIRE_GHOST || 
		 pm->gent->client->NPC_class == CLASS_BATFORM ||
		 pm->gent->client->NPC_class == CLASS_BATKAIN ||
		 pm->gent->client->NPC_class == CLASS_BATKAIN2 ||
		 pm->gent->client->NPC_class == CLASS_BATKAIN3 ||
		 pm->gent->client->NPC_class == CLASS_BATVORADOR ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_QUEEN ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_FROST || 
		 pm->gent->client->NPC_class == CLASS_BATNECROMANCER ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_WATER ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_ELF ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_ELF2 ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_ELF3 || 
		  pm->gent->client->NPC_class == CLASS_HELLBAT ||
		 pm->gent->client->NPC_class == CLASS_BATVAMPIRE_FEAR ||
		  pm->gent->client->NPC_class == CLASS_BATVAMPIRE_FEAR2 ||
		  pm->gent->client->NPC_class == CLASS_JANOS ||
		  pm->gent->client->NPC_class == CLASS_KAIN3 ||
		  pm->gent->client->NPC_class == CLASS_AIR ||
		  pm->gent->client->NPC_class == CLASS_VOID ||
		  pm->gent->client->NPC_class == CLASS_DARK ||
		  pm->gent->client->NPC_class == CLASS_LIGHT ||
		  pm->gent->client->NPC_class == CLASS_GOLEM_VOID ||
		  pm->gent->client->NPC_class == CLASS_GOLEM_AIR ||
		  pm->gent->client->NPC_class == CLASS_NAERYANMIST ||
		  pm->gent->client->NPC_class == CLASS_NAERYANMIST2 ||
		  pm->gent->client->NPC_class == CLASS_NAERYANMIST3 ||
		  pm->gent->client->NPC_class == CLASS_MISTFORM ||
		  pm->gent->client->NPC_class == CLASS_NAERYANMIST ||
		  pm->gent->client->NPC_class == CLASS_NAERYANMIST2 ||
		  pm->gent->client->NPC_class == CLASS_NAERYANMIST3 ||
		  pm->gent->client->NPC_class == CLASS_MISTKAIN ||
		  pm->gent->client->NPC_class == CLASS_MISTKAIN2 ||
		  pm->gent->client->NPC_class == CLASS_MISTKAIN3 ||
		  pm->gent->client->NPC_class == CLASS_MISTVORADOR ||
		  pm->gent->client->NPC_class == CLASS_MISTJANOS ||
		  pm->gent->client->NPC_class == CLASS_MISTNECROMANCER ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_QUEEN ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_DARK ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_FEAR ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_FEAR2 ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_BLOOD ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_WATER ||
		  pm->gent->client->NPC_class == CLASS_HELLMIST ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_SARAFAN ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_ELF ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_ELF2 ||
		  pm->gent->client->NPC_class == CLASS_MISTVAMPIRE_ELF3 ) 
	...
	if ( Flying == FLY_VEHICLE /*||
		pm->gent->client->NPC_class == CLASS_FIGHTER */)
	{
		if ( !(pm->ps->pm_flags & PMF_TIME_KNOCKBACK) && !(pm->ps->pm_flags & PMF_TIME_NOFRICTION) ) 
		{
			control = speed < pm_stopspeed ? pm_stopspeed : speed;
...
{//FIXME: ugh, hacky, set a flag on NPC or something, please...
	...
	if ( gent && gent->client && 
		...
		// gent->client->NPC_class == CLASS_FIGHTER ||
		// GOLEM TOO, CANNOT JUMP!! 
		gent->client->NPC_class == CLASS_GOLEM || 
		gent->client->NPC_class == CLASS_GOLEM_DARK || 
		gent->client->NPC_class == CLASS_GOLEM_SMOKE || 
		//gent->client->NPC_class == CLASS_GOLEM_VOID || 
		gent->client->NPC_class == CLASS_GOLEM_LIGHT || 
		gent->client->NPC_class == CLASS_GOLEM_GOLD || 
		gent->client->NPC_class == CLASS_GOLEM_FIRE || 
		gent->client->NPC_class == CLASS_GOLEM_LAVA || 
		gent->client->NPC_class == CLASS_HELLGOLEM || 
		//gent->client->NPC_class == CLASS_GOLEM_AIR || 
		gent->client->NPC_class == CLASS_GOLEM_METAL || 
		gent->client->NPC_class == CLASS_GOLEM_WATER ||
		gent->client->NPC_class == CLASS_GOLEM_ICE || 
		gent->client->NPC_class == CLASS_GOLEM_SONIC ||
		gent->client->NPC_class == CLASS_GOLEM_POISON ||
		gent->client->NPC_class == CLASS_GOLEM_HOLY ||
		gent->client->NPC_class == CLASS_GOLEM_NECRO ) )
	{
		return qtrue;
	}
	return qfalse;
...
static qboolean PM_CheckJump( void ) 
{
	...
	*/
	...
	if ( (pm->ps->clientNum < MAX_CLIENTS||PM_ControlledByPlayer()) 
		...
		&& (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER ||
		pm->gent->client->NPC_class == CLASS_RAZIELHIM || pm->gent->client->NPC_class == CLASS_ESKANDOR ||
		// BAT FORMS AND OTHER WINGED CREATURES 
		pm->gent->client->NPC_class == CLASS_BIRD ||
		 pm->gent->client->NPC_class == CLASS_POLTER ||
		 pm->gent->client->NPC_class == CLASS_REAPER ||
		 pm->gent->client->NPC_class == CLASS_ARIEL ||
								...
								if ( pm->cmd.forwardmove > 0 )
								{
									...
									*/
									if ( pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ALORA && Q_irand( 0, 3 ) ||
										pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VAMPIRE_WATER && Q_irand( 0, 3 ) )
									{
...
static void PM_CrashLand( void ) 
{
	...
	else
	{
		  ...
		  pm->gent->client->NPC_class == CLASS_GOLEM_AIR ) )
		{
			if ( JET_Flying( pm->gent ) )
			{
				if ( pm->gent->client->NPC_class == CLASS_BOBAFETT
		            ...
		            pm->gent->client->NPC_class == CLASS_GOLEM_AIR ||
					pm->gent->client->NPC_class == CLASS_GOLEM_VOID 	) )
					// DISATTIVA DANNO DI CADUTA SE IL PERSONAGGIO VOLA, FACENDOLO ATTERRARE SUI PIEDI 
				{
					JET_FlyStop( pm->gent );
				}
				else
				{
					// MIST IS NOT AFFECTED BY GRAVITY
					if ( pm->gent->client->NPC_class == CLASS_NAERYANMIST ||
						pm->gent->client->NPC_class == CLASS_NAERYANMIST2 ||
						pm->gent->client->NPC_class == CLASS_NAERYANMIST3 ||
						pm->gent->client->NPC_class == CLASS_MISTFORM ||
						pm->gent->client->NPC_class == CLASS_MISTKAIN ||
		...
		else if ( pm->ps->jumpZStart && (PM_InForceFall()) && (pm->ps->forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1||
										...
										if ( !pm->ps->forceJumpZStart || pm->ps->forceJumpZStart > pm->ps->origin[2] )// && fabs(pm->ps->velocity[0])<10 && fabs(pm->ps->velocity[1])<10 && pm->ps->velocity[2]<0)//either not force-jumping or force-jumped and now fell below original jump start height
										{
											...
											else if ( !trace.allsolid && !trace.startsolid && (pm->ps->origin[2] - trace.endpos[2]) >= 128 )//>=128 so we don't die on steps!
											{
												...
												{//didn't hit, we're probably going to die
													...
													&& pm->gent->client->NPC_class != CLASS_BIRD && pm->gent->client->NPC_class != CLASS_POLTER && 
													pm->gent->client->NPC_class != CLASS_REAPER &&
													pm->gent->client->NPC_class != CLASS_ARIEL &&
													pm->gent->client->NPC_class != CLASS_VAMPIRE_GHOST && 
													pm->gent->client->NPC_class != CLASS_BATFORM &&
	...
	if ( charging )
	{
		...
		if ( altFire )
		{
			if ( pm->ps->weaponstate != WEAPON_CHARGING_ALT && pm->ps->weaponstate != WEAPON_DROPPING )
			{
				...
				if ( cg.weaponSelect == WP_BLASTER_PISTOL )
				{
					if ( pm->gent->client && pm->gent->client->gs.gun->Gun_Alt_Charge_Sound )
					{
						// custom select sound
						G_SoundOnEnt( pm->gent, CHAN_WEAPON, pm->gent->client->gs.gun->Gun_Alt_Charge_Sound );
						//cgi_S_StartSound (NULL, es->number, CHAN_AUTO, cgi_S_RegisterSound( cent->gent->client->gs.gun->Gun_Select_Sound ));
					}
...
#define BLOODGUN_CHARGE_UNIT	200.0f
#define DEMP2_CHARGE_UNIT		500.0f	// ditto
#define POISON_CHARGE_UNIT		400.0f	// ditto
	...
	if ( pm->ps->weapon == WP_BOWCASTER && !( pm->cmd.buttons & BUTTON_ALT_ATTACK ))
	{
		...
		*amount *= count;
			  ...
			  || ( pm->ps->weapon == WP_BLASTER_PISTOL && pm->cmd.buttons & BUTTON_ALT_ATTACK ) 
					|| ( pm->ps->weapon == WP_POISON && pm->cmd.buttons & BUTTON_ALT_ATTACK )
     )
	{
...
extern void ForceStun( gentity_t *self );
extern void ForceHate( gentity_t *self );
extern void ForceControlMind( gentity_t *self );
extern void ForceHealOther( gentity_t *self );
extern void ForceWrack (gentity_t *self); 
extern void ForceFreeze( gentity_t *self );
extern void ForceGlyph( gentity_t *self );
extern void StoneGlyph( gentity_t *self );
extern void SoundGlyph( gentity_t *self );
extern void WaterGlyph( gentity_t *self );
extern void FireGlyph (gentity_t *self); 
extern void SunGlyph( gentity_t *self );
extern void WindGlyph( gentity_t *self );
extern void HolyGlyph( gentity_t *self );
extern void DarkGlyph (gentity_t *self); 
extern void NecroGlyph( gentity_t *self );
...
void PM_SetSpecialMoveValues (void )
{
	...
	if ( pm->gent )
	{
		...
		// EDIT CODE TRY TO FLY OLON AND FIGHTERS NPC LIKE VEHICLE FIGHTERS 
		else if ( //pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_FIGHTER ||
			pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_OLON )
		{
			Flying = FLY_VEHICLE;
		}
	}
...
qboolean PM_FormsWeapon ( int weapon )
{
...
void Pmove( pmove_t *pmove )
{
	...
	// EDIT CODE TRY TO ACTIVATE FLY BEHAVOUR FOR OLON AND FIGHTER WITH THE FLYVEHICLE! 
	else if ( Flying == FLY_VEHICLE /*|| pm->gent->client->NPC_class == CLASS_FIGHTER ||
		pm->gent->client->NPC_class == CLASS_OLON */)
	{
		PM_FlyVehicleMove();
	}
