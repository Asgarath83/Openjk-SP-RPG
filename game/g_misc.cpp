...
void shield_power_converter_use( gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if ( self->setTime < level.time )
	{
		...
		dif = 999 - activator->client->ps.stats[STAT_ARMOR]; // FIXME: define for max armor?
		...
		{  
			activator->client->ps.stats[STAT_ARMOR] = activator->client->ps.stats[STAT_MAX_ARMOR];
			G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
			self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
			self->s.loopSound = 0;
		...
		/*
		//give your amount of armor only in relation at your armour level. 
		switch  ( activator->client->ARMORLEVEL )
		{
		   case ARMOR_NONE:
		   {
			  if ( activator->client->ps.stats[STAT_ARMOR] >= 0 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR1:
		   {
				if ( activator->client->ps.stats[STAT_ARMOR] >= 100 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR2:
		   {
				if ( activator->client->ps.stats[STAT_ARMOR] >= 120 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR3:
		   {
			   if ( activator->client->ps.stats[STAT_ARMOR] >= 150 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR4:
		   {
				if ( activator->client->ps.stats[STAT_ARMOR] >= 200 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR5:
		   {
				 if ( activator->client->ps.stats[STAT_ARMOR] >= 250 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR6:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 300 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR7:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 350 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR8:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 400 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR9:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 450 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR10:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 500 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR11:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 550 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR12:
		   {
			  if ( activator->client->ps.stats[STAT_ARMOR] >= 600 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR13:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 650 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR14:
		   {
				if ( activator->client->ps.stats[STAT_ARMOR] >= 700 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR15:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 750 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR16:
		   {
			  if ( activator->client->ps.stats[STAT_ARMOR] >= 800 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR17:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 850 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR18:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 900 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   break;
		    case ARMOR19:
		   {
			 if ( activator->client->ps.stats[STAT_ARMOR] >= 950 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
		   ...
		   case ARMOR20:
		   {
			   if ( activator->client->ps.stats[STAT_ARMOR] >= 1000 ) // FIXME: define for max
				{
					// play full sound
					G_Sound( self, G_SoundIndex( "sound/interface/shieldcon_done.mp3" ));
					self->setTime = level.time + 1000; // extra debounce so that the sounds don't overlap too much
...
extern void G_DriveOlon( gentity_t *ent, gentity_t *fighter );
...
void misc_olon_use( gentity_t *self, gentity_t *other, gentity_t *activator )
{
	if ( !activator || activator->s.number )
	{//only player can do this
		return;
	...
	if ( activator->client->NPC_class != CLASS_OLON )
	{//get in the Fighter
		//copy origin
		G_SetOrigin( activator, self->currentOrigin );
		...
		//set player to my g2 instance
		gi.G2API_StopBoneAnimIndex( &self->ghoul2[self->playerModel], self->craniumBone );
		G_DriveOlon( activator, self ); // DRIVE THE OLON!!! 
		activator->activator = self;
		self->s.eFlags |= EF_NODRAW; // Hide player
		self->svFlags |= SVF_NOCLIENT; // Hide player 
		self->contents = 0;
		self->takedamage = qfalse; // maybe is better to uncheck that... should be cause of olon invulnerability
		//transfer armor
		tempHealth = self->health;
		self->health = activator->client->ps.stats[STAT_ARMOR];
		activator->client->ps.stats[STAT_ARMOR] = tempHealth;
	...
	{//get out of ATST
		...
		//misc_atst_setanim( self, self->craniumBone, BOTH_STAND2 );
		//gi.G2API_SetSurfaceOnOff( &self->ghoul2[self->playerModel], "head_hatchcover", 0 );
		G_Sound( self, G_SoundIndex( "sound/chars/fighter/fighter_hatch_open" ));
	}*/
}
...
void SP_misc_olon_drivable( gentity_t *ent )
{
	extern void NPC_ATST_Precache(void);
	extern void NPC_PrecacheAnimationCFG( const char *NPC_type );
