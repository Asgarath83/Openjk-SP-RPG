...
extern void WP_GunFreeStrings( gunData_t &gun );
void SP_info_player_deathmatch(gentity_t *ent) {
	...
	else
	{
		...
		// Register Gun 
		RegisterItem( FindItemForWeapon( WP_BLASTER_PISTOL ) );	// Assigned to Blaster Pistol weapon 
		gunData_t	gun;
		WP_GunParseParms( g_gun->string, &gun );//get gun stuff cached before client begins
		// IT CRASH FOR MOMENT.. 
		if (gun.Gun_Model) G_ModelIndex( gun.Gun_Model );
		if (gun.Gun_Skin) G_SkinIndex( gun.Gun_Skin );
		WP_GunFreeStrings(gun); //IT CRASH FOR MOMENT. 
...
*/
	...
	if ( client->pers.maxHealth < 1 || client->pers.maxHealth > 999 ) // was 100
	{
		if ( client->PLAYERLEVEL )
		{ 
			switch ( client->PLAYERLEVEL )
			{
				...
				case LEVEL7:  
				{
					client->pers.maxHealth = 170;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL8:  
				{
					client->pers.maxHealth = 180;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL9:  
				{
					client->pers.maxHealth = 190;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL10:  
				...
				case LEVEL11:  
				{
					client->pers.maxHealth = 210;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL12:  
				{
					client->pers.maxHealth = 220;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL13:  
				{
					client->pers.maxHealth = 230;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL14:  
				...
				case LEVEL16:  
				{
					client->pers.maxHealth = 260;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL17:  
				{
					client->pers.maxHealth = 270;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL18:  
				{
					client->pers.maxHealth = 280;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL19:  
				{
					client->pers.maxHealth = 290;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL20:  
				...
				case LEVEL21:  
				{
					client->pers.maxHealth = 310;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL22:  
				{
					client->pers.maxHealth = 320;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL23:  
				{
					client->pers.maxHealth = 330;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL24:  
				{
					client->pers.maxHealth = 340;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL25:  
				{
					client->pers.maxHealth = 350;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL26:  
				{
					client->pers.maxHealth = 360;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL27:  
				{
					client->pers.maxHealth = 370;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL28:  
				{
					client->pers.maxHealth = 380;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL29:  
				{
					client->pers.maxHealth = 390;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL30:  
				...
				case LEVEL31:  
				{
					client->pers.maxHealth = 410;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL32:  
				{
					client->pers.maxHealth = 420;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL33:  
				{
					client->pers.maxHealth = 430;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL34:  
				{
					client->pers.maxHealth = 440;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL35:  
				{
					client->pers.maxHealth = 450;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL36:  
				{
					client->pers.maxHealth = 460;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL37:  
				{
					client->pers.maxHealth = 470;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL38:  
				{
					client->pers.maxHealth = 480;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL39:  
				{
					client->pers.maxHealth = 490;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL40:  
				...
				case LEVEL41:  
				{
					client->pers.maxHealth = 510;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL42:  
				{
					client->pers.maxHealth = 520;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL43:  
				{
					client->pers.maxHealth = 530;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL44:  
				{
					client->pers.maxHealth = 540;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL45:  
				{
					client->pers.maxHealth = 550;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL46:  
				{
					client->pers.maxHealth = 560;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL47:  
				{
					client->pers.maxHealth = 570;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL48:  
				{
					client->pers.maxHealth = 580;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL49:  
				{
					client->pers.maxHealth = 590;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL50:  
				{
					client->pers.maxHealth = 600;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL51:  
				{
					client->pers.maxHealth = 610;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL52:  
				{
					client->pers.maxHealth = 620;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL53:  
				{
					client->pers.maxHealth = 630;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL54:  
				{
					client->pers.maxHealth = 640;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL55:  
				{
					client->pers.maxHealth = 650;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL56:  
				{
					client->pers.maxHealth = 660;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL57:  
				{
					client->pers.maxHealth = 670;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL58:  
				{
					client->pers.maxHealth = 680;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL59:  
				{
					client->pers.maxHealth = 690;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL60:  
				{
					client->pers.maxHealth = 700;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL61:  
				{
					client->pers.maxHealth = 710;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL62:  
				{
					client->pers.maxHealth = 720;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL63:  
				{
					client->pers.maxHealth = 730;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL64:  
				{
					client->pers.maxHealth = 740;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL65:  
				{
					client->pers.maxHealth = 750;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL66:  
				{
					client->pers.maxHealth = 760;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL67:  
				{
					client->pers.maxHealth = 770;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL68:  
				{
					client->pers.maxHealth = 780;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL69:  
				{
					client->pers.maxHealth = 790;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL70:  
				{
					client->pers.maxHealth = 800;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL71:  
				{
					client->pers.maxHealth = 810;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL72:  
				{
					client->pers.maxHealth = 820;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL73:  
				{
					client->pers.maxHealth = 830;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL74:  
				{
					client->pers.maxHealth = 840;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL75:  
				{
					client->pers.maxHealth = 850;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL76:  
				{
					client->pers.maxHealth = 860;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL77:  
				{
					client->pers.maxHealth = 870;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL78:  
				{
					client->pers.maxHealth = 880;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL79:  
				{
					client->pers.maxHealth = 890;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL80:  
				{
					client->pers.maxHealth = 900;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL81:  
				{
					client->pers.maxHealth = 910;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL82:  
				{
					client->pers.maxHealth = 920;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL83:  
				{
					client->pers.maxHealth = 930;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL84:  
				{
					client->pers.maxHealth = 940;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL85:  
				{
					client->pers.maxHealth = 950;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL86:  
				{
					client->pers.maxHealth = 960;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL87:  
				{
					client->pers.maxHealth = 970;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL88:  
				{
					client->pers.maxHealth = 980;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL89:  
				{
					client->pers.maxHealth = 990;
					client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
				}
				break; 
				case LEVEL90:
		...
		{
			client->pers.maxHealth = 100;//was 999
			client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
		}
	}
...
void G_AddWeaponModels( gentity_t *ent )
{
	...
	if ( ent->weaponModel[0] == -1 )
	{
		...
		else if ( ent->client->ps.weapon == WP_BLASTER_PISTOL ) 
		{
...
void G_SetGunsFromCVars( gentity_t *ent )
{
	if ( g_gun->string
		&& g_gun->string[0]
		&& Q_stricmp( "none", g_gun->string )
		&& Q_stricmp( "NULL", g_gun->string ) )
	{//FIXME: how to specify second saber?
		WP_GunParseParms( g_gun->string, &ent->client->gs.gun[0] );
	}

	if ( g_gun2->string 
		&& g_gun2->string[0]
		&& Q_stricmp( "none", g_gun2->string )
		&& Q_stricmp( "NULL", g_gun2->string ) )
	{
		if ( !(ent->client->gs.gun[0].gunFlags3&GFL_TWOGUNS) )
		{//can't use a second gun if first one is a two-handed gun...?
			WP_GunParseParms( g_gun2->string, &ent->client->gs.gun[1] );
			if ( (ent->client->gs.gun[1].gunFlags3&GFL_TWOGUNS) )
			{//tsk tsk, can't use a twoHanded gun as second gun
				WP_RemoveGun( ent, 1 );
			}
			else
			{
...
void G_ReloadGunData( gentity_t *ent )
{
	//dualguns should already be set
	if ( ent->client->gs.gun[0].Gun_Name != NULL )
	{
		WP_GunParseParms( ent->client->gs.gun[0].Gun_Name, &ent->client->gs.gun[0] );
	}
	if ( ent->client->gs.gun[1].Gun_Name != NULL )
	{
		WP_GunParseParms( ent->client->gs.gun[1].Gun_Name, &ent->client->gs.gun[1] );
	}
}
...
qboolean ClientSpawn(gentity_t *ent, SavedGameJustLoaded_e eSavedGameJustLoaded ) 
{
	...
	{//loading up a full save game
		...
		//G_ReloadGunData( ent ); // load gundata
	}
	else
	{
		// find a spawn point
...
//			}
		...
		if ( ent->client->ps.weapons[WP_BLASTER_PISTOL] )
		{//set up so has lightsaber
			WP_GunInitData( ent );
			if ( (ent->weaponModel[0] <= 0 || (ent->weaponModel[1]<=0&&ent->client->gs.dualGuns)) //one or both of the gun models is not initialized
				&& ent->client->ps.weapon == WP_BLASTER_PISTOL )//current weapon is saber
			{//add the proper models
				//WP_SaberAddG2SaberModels( ent );// temp. 
				WP_GunAddG2GunModels( ent );
			}
		}
