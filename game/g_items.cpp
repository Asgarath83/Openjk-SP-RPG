...
// WEAPON SWAPPING CVARS SYSTEM
extern cvar_t 	*g_gunSwapPistols;
...
// Drop Pistol Item - Create Blaster Pistol Item from enemy Killed. 
void G_CopyPistolItemValues( gentity_t *pickUpPistol, gentity_t *oldPistol )
{
	if ( oldPistol && pickUpPistol )
	{
		oldPistol->spawnflags = pickUpPistol->spawnflags;
		oldPistol->random = pickUpPistol->random;
		oldPistol->flags = pickUpPistol->flags;
	}
}



gentity_t *G_DropPistolItem( const char *PistolType, vec3_t PistolPos, vec3_t PistolVel, vec3_t PistolAngles, gentity_t *copyPistol )
{//turn it into a pick-uppable item!
	gentity_t *newItem = NULL;
	if ( PistolType
		&& PistolType[0] )
	{//have a valid string to use for saberType
		...
		if ( newItem )
		{
			newItem->classname = G_NewString( "weapon_blaster_pistol" );
			VectorCopy( PistolPos, newItem->s.origin );
			G_SetOrigin( newItem, newItem->s.origin );
			VectorCopy( PistolAngles, newItem->s.angles );
			G_SetAngles( newItem, newItem->s.angles );
			newItem->spawnflags = 128;/*ITMSF_USEPICKUP*/
			newItem->spawnflags |= 64;/*ITMSF_NOGLOW*/
			newItem->NPC_type = G_NewString( PistolType );//saberType
			//FIXME: transfer per-blade color somehow?
			//newItem->NPC_targetname = (char *)saberColorStringForColor[saberColor];
			newItem->count = 0; // no new ammo added. collect ammo by died enemies and ammoes racks)
			newItem->flags = FL_DROPPED_ITEM;
			G_SpawnItem( newItem, FindItemForWeapon( WP_BLASTER_PISTOL ) );
			newItem->s.pos.trType = TR_GRAVITY;
			newItem->s.pos.trTime = level.time;
			VectorCopy( PistolVel, newItem->s.pos.trDelta );
			//newItem->s.eFlags |= EF_BOUNCE_HALF;
			//copy some values from another saber, if provided:
			G_CopyPistolItemValues( copyPistol, newItem );
			//don't *think* about calling FinishSpawningItem, just do it!
			newItem->e_ThinkFunc = thinkF_NULL;
			newItem->nextthink = -1;
			FinishSpawningItem( newItem );
...
extern void G_SetGunsFromCVars ( gentity_t *ent );
...
// SWAP BLASTER PISTOL FUNCTION
qboolean Pickup_Pistol( gentity_t *self, qboolean hadPistol, gentity_t *pickUpPistol )
{
	qboolean foundIt = qfalse;

	if ( !pickUpPistol || !self || !self->client )
	{
		return qfalse;
	}
	...
	if ( Q_stricmp( "player", pickUpPistol->NPC_type ) == 0 )
	{//"player" means use cvar info
		G_SetGunsFromCVars( self );
		foundIt = qtrue;
	}
	else 
	{
		gunData_t	newPistol={0};
		qboolean swapPistols = qfalse;

		if ( self->client->ps.weapon == WP_BLASTER_PISTOL
			&& self->client->ps.weaponTime > 0 )
		{//can't pick up a new saber while the old one is busy (also helps to work as a debouncer so you don't swap out sabers rapidly when touching more than one at a time)
			return qfalse;
		}

		if ( pickUpPistol->count == 1
			&& g_gunSwapPistols->integer )
		{
			swapPistols = qtrue;
		}

		if ( WP_GunParseParms( pickUpPistol->NPC_type, &newPistol ) )
		{//successfully found a pistol .gun entry to use
			int	PistolNum = 0;
			qboolean removeLeftPistol = qfalse;
			if ( !hadPistol )
			{//don't have a pistol at all yet, put it in our right hand
				PistolNum = 0;
				//just in case...
				removeLeftPistol = qtrue;
			}
			else if ( (newPistol.gunFlags3&GFL_TWOGUNS) //new pistol is two-handed
				|| (hadPistol && (self->client->gs.gun[0].gunFlags3&GFL_TWOGUNS)) )//old saber is two-handed
			{//replace the old right-hand saber and remove the left hand one
				// put two pistol of the same kind in the hand! 
				PistolNum = 0;
				PistolNum = 1;
				removeLeftPistol = qtrue;
			}
			else
			{//have, at least, a pistol in our right hand and the new one could go in either left or right hand
				//just add it as a new pistol
					PistolNum = 0;
					//just in case...
					removeLeftPistol = qtrue;
			}
			...
			{//want to reach out with right hand
				...
				if ( swapPistols )
				{//drop first one where the one we're picking up is
					G_DropPistolItem( self->client->gs.gun[PistolNum].Gun_Name, pickUpPistol->currentOrigin, (float *)vec3_origin, pickUpPistol->currentAngles, pickUpPistol );
					if ( removeLeftPistol )
					{//drop other one at my origin
						G_DropPistolItem( self->client->gs.gun[1].Gun_Name, self->currentOrigin, (float *)vec3_origin, self->currentAngles, pickUpPistol );
					}
				}
			}
			else
			...
			WP_SetGun( self, PistolNum, pickUpPistol->NPC_type );
			WP_GunInitData ( self );
			if ( self->client->ps.torsoAnim == BOTH_BUTTON_HOLD 
				|| self->client->ps.torsoAnim == BOTH_SABERPULL )
			{//don't let them attack right away, force them to finish the anim
	...
	*/
	...
	{//NPC
		...
		{//NPC with no weapon picked up a weapon, change to this weapon
			...
			else if ( ent->item->giTag == WP_BLASTER_PISTOL )
			{
...
void FinishSpawningItem( gentity_t *ent ) {
	...
	if ( ent->item->giType == IT_WEAPON
		&& ent->item->giTag == WP_BLASTER_PISTOL
		&& ent->NPC_type
		&& ent->NPC_type[0] )
	{
		gunData_t ItemGun;
		...
		{//player's gun
			WP_GunParseParms( g_gun->string, &ItemGun );
		}
		else
		{//specific gun
			WP_GunParseParms( ent->NPC_type, &ItemGun );
		}
		//NOTE:  should I keep this string around for any reason?  Will I ever need it later?
		//ent->??? = G_NewString( itemSaber.model );
		gi.G2API_InitGhoul2Model( ent->ghoul2, ItemGun.Gun_Model, G_ModelIndex( ItemGun.Gun_Model ), NULL_HANDLE, NULL_HANDLE, 0, 0);
		WP_GunFreeStrings(ItemGun);
	}
	else
	{
