...
typedef struct levelState_s {

	int			PlayerLevel[NUM_LEVELS];
	int			PlayerArmorLevel[NUM_ARMORS];
} levelState_t;

// WEAPON CODE STUFF 
#define MAX_GUNS 2 
typedef struct gunState_s {
	
 gunData_t	gun[MAX_GUNS];
 int			gunEntityNum;
 qboolean	gunInFlight;
 float		gunEntityDist;
 int		gunEntityState; 
 short		gunMove;
short		gunMoveNext;
...
#define GUNENTITYSTATE_LEAVING		1
...
//icarus forced moving.  is this still used?
	...
	//healed 
	int			healTime;					// amount of time heal regen with a charm
	int			healPoints;					// amount of HP got by a charm. 
	//choked
	int			smokeDamage;
	int			smokeTime; 
	// void space dmg 
	...
	int			hyldenDamage;
	int			hyldenTime; 
	// Hylden lightning dmg
	int			hyldenShockDamage; 
	int			hyldenShockTime; 
	// Sonic dmg 
	int			sonicDamage;
	int			sonicTime; 
	...
	int			breakCounter;			//how many defense points have we used/strong attacks have we blocked recently?
 	//qboolean	saberDropped;
	levelState_t	ls;				// hopefully that not cause the issue of no dmg from weapons! 
	gunState_t	gs;				// communicated by server to clients
};
...
//FIXME: Are these being used anymore?
	...
	short			gunModel[MAX_INHAND_GUNS];
	// RPG CODE.
	int			armor;
	int			max_armor;
	// For better customization of Means of Damage:
	int			methodOfDeath2;
	int			splashMethodOfDeath2;
	int			methodOfDeath3;
	int			splashMethodOfDeath3;
};
...
// each *GUNS Pistol has an associated GunInfo_t
// that contains media references necessary to present the
// weapon 
typedef struct GunInfo_s {
	qboolean		Gun_Registered;
	gitem_t			*gun_item;
	qhandle_t		Gun_HandsModel;			// the hands don't actually draw, they just position the weapon
	qhandle_t		Gun_WeaponModel;		//for in view
	qhandle_t		Gun_WeaponWorldModel;	//for in their hands
	qhandle_t		Gun_BarrelModel[4];
	vec3_t			Gun_Midpoint;		// so it will rotate centered instead of by tag
	qhandle_t		Gun_Icon;			// The version of the icon with a glowy background
	qhandle_t		Gun_IconNoAmmo;	// The version of the icon with no ammo warning
	qhandle_t		Gun_AmmoIcon;
	qhandle_t		Gun_AmmoModel;
	qhandle_t		Gun_Missile_Model;
	sfxHandle_t		Gun_Missile_Sound;
	void			(*Gun_Missile_Trail_Func)( centity_t *, const struct GunInfo_s *gi );
	qhandle_t		Gun_Alt_Missile_Model;
	sfxHandle_t		Gun_Alt_Missile_Sound;
	void			(*Gun_Alt_Missile_Trail_Func)( centity_t *, const struct GunInfo_s *gi );
	sfxHandle_t		Gun_Charge_Sound;
	sfxHandle_t		Gun_Alt_Charge_Sound;
	sfxHandle_t		Gun_Select_Sound;	// sound played when weapon is selected
	sfxHandle_t		Gun_Loop_Sound; // sound looped by gun
	// EFX registering
	int		Gun_Muzzle_Effect_Registered; // Main Shoot Muzzle Effect 
	int		Gun_Alt_Muzzle_Effect_Registered; // Alt Shoot Muzzle Effect
	int		Gun_Bullet_Effect_Registered; // Main Shoot Muzzle Effect 
	int		Gun_Alt_Bullet_Effect_Registered; // Alt Shoot Muzzle Effect
    int		Gun_Ray_Effect_Registered;	// Shader of main gun ray
	int		Gun_Alt_Ray_Effect_Registered;	// Shader of alt gun ray
	int 	Gun_WallImpact_Effect_Registered; // Main Shoot Muzzle Effect 
	int		Gun_Alt_WallImpact_Effect_Registered; // Alt Shoot Muzzle Effect
	int 	Gun_FleshImpact_Effect_Registered; // Main Shoot Muzzle Effect 
	int		Gun_Alt_FleshImpact_Effect_Registered; // Alt Shoot Muzzle Effect
	int 	Gun_Bounce_Effect_Registered; // Main Shoot Muzzle Effect 
	int		Gun_Alt_Bounce_Effect_Registered; // Alt Shoot Muzzle Effect
	int		Gun_Charge_Shader_Effect_Registered; // effect draw on barrel when weapon charge the shoot
	int		Gun_Alt_Charge_Shader_Effect_Registered; // effect draw on barrel wehn weapon charge alt shoot. 
	int		Gun_Disgregate_Effect_Registered;	// effect of disintegration
	int		Gun_Alt_Disgregate_Effect_Registered; // alt effect of disintegration
	int		Gun_Poison_Effect_Registered;	// Set Visible effect of poison of enemy
	int		Gun_Alt_Poison_Effect_Registered; // Set Alt Visible Effect of Poison of Enemy
} GunInfo_t;
