...
extern void G_RemoveGunModels( gentity_t *pEnt );
...
void G_DrivableOlonDie( gentity_t *self )
{
}
...
void G_DriveOlon( gentity_t *pEnt, gentity_t *Olon )
{
	if ( pEnt->NPC_type && pEnt->client && (pEnt->client->NPC_class == CLASS_OLON) )
	{//already an Olon, switch back 
		//UNCHECKED BECAUSE IS UNNECESSARY STUFF 
		//open hatch
		/*
		G_RemovePlayerModel( pEnt );
		pEnt->NPC_type = "player";
	...
	{//become an atst
		pEnt->NPC_type = "Olon";
		pEnt->client->NPC_class = CLASS_OLON;
		pEnt->client->ps.eFlags |= EF_IN_ATST;// Suppose activate HUD etc 
		//pEnt->flags |= FL_SHIELDED; // NO SHIELDED GET DMG! DAMNED! 
		//size
		//VectorSet( pEnt->mins, ATST_MINS0, ATST_MINS1, ATST_MINS2 );
		//VectorSet( pEnt->maxs, ATST_MAXS0, ATST_MAXS1, ATST_MAXS2 );
		//pEnt->client->crouchheight = ATST_MAXS2;
		...
		else
		{
			...
			G_RemoveWeaponModels( pEnt );
			G_RemoveGunModels( pEnt );
			gi.G2API_CopyGhoul2Instance( Olon->ghoul2, pEnt->ghoul2, -1 );
			pEnt->playerModel = 0;
			G_SetG2PlayerModelInfo( pEnt, "atst", NULL, NULL, NULL );
			//turn off hatch underside
			gi.G2API_SetSurfaceOnOff( &pEnt->ghoul2[pEnt->playerModel], "head_hatchcover", 0x00000002/*G2SURFACEFLAG_OFF*/ );
		...
		gi.cvar_set( "cg_thirdperson", "1" );
		cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_RNG;
		cg.overrides.thirdPersonRange = 300;//240;
		//cg.overrides.thirdPersonVertOffset = 100;
		//cg.overrides.thirdPersonPitchOffset = -30;
		//FIXME: this gets stomped in pmove?
		pEnt->client->ps.viewheight = 160;//120; 
		//FIXME: setting these broke things very badly...?
		//pEnt->client->standheight = 200;
		//pEnt->client->crouchheight = 200;
		//pEnt->mass = 300;
		//movement
		// HOW CAN FLU THIS... THING??? 
	}
}
#endif //_JK2MP
