...
//Add function for fighters 
extern void SP_misc_atst_drivable( gentity_t *ent ); 
extern void SP_misc_olon_drivable( gentity_t *ent ); 
//
...
void NPC_SetMiscDefaultData( gentity_t *ent )
{
	...
	{//if I'm equipped with a saber, initialize it (them)
		...
		{//this is our current gun, add the models now
	...
	case TEAM_PLAYER:
		...
		{// Drivable fighter vehicle - Player class starfighter 
			ent->NPC->defaultBehavior = BS_CINEMATIC; // don't attack! Not AI  
			ent->client->ps.gravity = 0; // No gravity
			ent->svFlags |= SVF_CUSTOM_GRAVITY; // no gravity
			ent->client->moveType = MT_FLYSWIM; // floating in air 
			ent->s.loopSound = G_SoundIndex( "sound/vehicles/tie-bomber/loop.wav" ); // spawn with loopind engine sound
			extern void NPC_Fighter_Precache(void);  // precache fighters data 
			extern void NPC_PrecacheAnimationCFG( const char *NPC_type ); // not necessary, but i leave for moment. i think . a fighter has no animation.
			ent->rootBone = gi.G2API_GetBoneIndex( &ent->ghoul2[ent->playerModel], "model_root", qtrue ); // my fighter use humanoid skeleton and i guess need for movement, so okay... 
			...
			ent->s.radius = 320; // not sure about that
			VectorSet( ent->s.modelScale, 1.0f, 1.0f, 1.0f ); // not sure about that 

			//register my weapons, sounds and model
			RegisterItem( FindItemForWeapon( WP_BLASTER ));	//precache the weapon
			// ALL OTHER WEAPONS ARE LOADED ONLY IF PLAYER IS EQUIPPED WITH THAT! 
			//HACKHACKHACKTEMP - until ATST gets real weapons of it's own?
			// OLON IS FULLY EQUIPPED WITH
			//	RegisterItem( FindItemForWeapon( WP_BLASTER_PISTOL ));	//Stunner precache the weapon
			//	RegisterItem( FindItemForWeapon( WP_BOWCASTER ));	//concussion rockets precache the weapon
			//	RegisterItem( FindItemForWeapon( WP_ROCKET_LAUNCHER ));	//seekers missiles
			//	RegisterItem( FindItemForWeapon( WP_REPEATER ));//bombs
			//	RegisterItem( FindItemForWeapon( WP_FLECHETTE ));//chaff weapon
			//	RegisterItem( FindItemForWeapon( WP_CONCUSSION ));//antimatter beam
			// RegisterItem( FindItemForWeapon( WP_DEMP2 ));//em pulse weapon for stun fighters 
			...
			//ent->flags |= FL_SHIELDED; NOPE! it can be damaged easily! 
			...
			if ( !ent->health )
	{
			...
			ent->e_UseFunc = useF_misc_olon_use; // Should create a fighter use function 
			ent->svFlags |= SVF_PLAYER_USABLE;
			
			//make it able to take damage and die when you're not in it...
			//do an explosion and play the death anim, remove use func.
			ent->e_DieFunc = dieF_misc_atst_die; // Should create at end a die fighter function*/
		}
