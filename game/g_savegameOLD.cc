...
// ADD HERE THE ULTIMATE OPEN JK CODE FOR IMPROVE SAVEGAME STRUCTURE AT THE END
// OF GUN CODE //////////
...
extern void G_ReloadGunData( gentity_t *ent );
...
*/
	{strCLOFS(gs.gun[0].Gun_Name),F_STRING},	
	{strCLOFS(gs.gun[1].Gun_Name),F_STRING},
...
extern void WP_GunSetDefaults ( GunInfo_t *gun, qboolean setGuns = qtrue ); 
...
typedef struct
{
	...
	char			*Gun_Missile_Sound;		// Sound made when weapon stops firing
	char			*Gun_Alt_Missile_Sound;
	...
	// UI - THEY WAS CHARS 
	char	*Gun_Icon;		// Name of weapon icon file
	char	*Gun_Desc;		// String Descriprion of Gun
	char	*Gun_DisplayName; // The name which is to be displayed on the HUD.
...
#ifndef _USRDLL
	// Visual effects 
	char 	*Gun_Muzzle_Effect; // Main Shoot Muzzle Effect 
	char	*Gun_Alt_Muzzle_Effect; // Alt Shoot Muzzle Effect
	char 	*Gun_Bullet_Effect; // Main Shoot Muzzle Effect 
	char 	*Gun_Alt_Bullet_Effect; // Alt Shoot Muzzle Effect
    char	*Gun_Ray_Effect;	// Shader of main gun ray
	char	*Gun_Alt_Ray_Effect;	// Shader of alt gun ray
	char 	*Gun_WallImpact_Effect; // Main Shoot Muzzle Effect 
	char	*Gun_Alt_WallImpact_Effect; // Alt Shoot Muzzle Effect
	char 	*Gun_FleshImpact_Effect; // Main Shoot Muzzle Effect 
	char	*Gun_Alt_FleshImpact_Effect; // Alt Shoot Muzzle Effect
	char 	*Gun_Bounce_Effect; // Main Shoot Muzzle Effect 
	char	*Gun_Alt_Bounce_Effect; // Alt Shoot Muzzle Effect
	char	*Gun_Charge_Shader_Effect; // effect draw on barrel when weapon charge the shoot
	char	*Gun_Alt_Charge_Shader_Effect; // effect draw on barrel wehn weapon charge alt shoot. 
	char	*Gun_Disgregate_Effect;	// effect of disintegration
	char	*Gun_Alt_Disgregate_Effect; // alt effect of disintegration
#endif
	// Damages 
	...
	char		*Gun_Loop_Sound;		// Loop Sound played by gun when is selected
...
static void SG_ConvertRetailGuninfoToNewGuninfo( void *gunData, GunInfo_t *GunNew )
{
	GunInfoRetail_t *GunRetail = ((GunInfoRetail_t *)(gunData));

	for ( int GunNum = 0; GunNum < 2; GunNum++ )
	{
		/*WP_GunSetDefaults( &GunNew[GunNum], qfalse );
		GunNew[GunNum].Gun_MagicRestriction = GunRetail[GunNum].Gun_MagicRestriction;
	
		GunNew[GunNum].Gun_Name = GunRetail[GunNum].Gun_Name;
		GunNew[GunNum].Gun_FullName = GunRetail[GunNum].Gun_FullName;
		GunNew[GunNum].Gun_Model = GunRetail[GunNum].Gun_Model;
		GunNew[GunNum].Gun_Skin = GunRetail[GunNum].Gun_Skin;
		// save informations about missile 
		GunNew[GunNum].Gun_Missile_Model = GunRetail[GunNum].Gun_Missile_Model;
		GunNew[GunNum].Gun_Alt_Missile_Model = GunRetail[GunNum].Gun_Alt_Missile_Model;
		GunNew[GunNum].Gun_Missile_Sound = GunRetail[GunNum].Gun_Missile_Sound;
		GunNew[GunNum].Gun_Alt_Missile_Sound = GunRetail[GunNum].Gun_Alt_Missile_Sound;
		GunNew[GunNum].Gun_Charge_Sound = GunRetail[GunNum].Gun_Charge_Sound;
		GunNew[GunNum].Gun_Alt_Charge_Sound = GunRetail[GunNum].Gun_Alt_Charge_Sound;
		GunNew[GunNum].Gun_Select_Sound = GunRetail[GunNum].Gun_Select_Sound;
		// missile stuff 		
		GunNew[GunNum].Gun_Icon = GunRetail[GunNum].Gun_Icon;
		GunNew[GunNum].Gun_Loop_Sound = GunRetail[GunNum].Gun_Loop_Sound;
		GunNew[GunNum].Gun_Select_Sound = GunRetail[GunNum].Gun_Select_Sound;
		GunNew[GunNum].Gun_Charge_Sound = GunRetail[GunNum].Gun_Charge_Sound;
		GunNew[GunNum].Gun_Type = GunRetail[GunNum].Gun_Type;
		// Ammo Stuff 
		GunNew[GunNum].Gun_AmmoMax = GunRetail[GunNum].Gun_AmmoMax;
		GunNew[GunNum].Gun_AmmoIndex = GunRetail[GunNum].Gun_AmmoIndex;
		GunNew[GunNum].Gun_AmmoLow = GunRetail[GunNum].Gun_AmmoLow;
		GunNew[GunNum].Gun_AmmoPerShot = GunRetail[GunNum].Gun_AmmoPerShot;
		GunNew[GunNum].Gun_AltAmmoPerShot = GunRetail[GunNum].Gun_AltAmmoPerShot;
		// UI - THEY WAS CHARS 
		GunNew[GunNum].Gun_Icon = GunRetail[GunNum].Gun_Icon;
		GunNew[GunNum].Gun_Desc = GunRetail[GunNum].Gun_Desc;
		GunNew[GunNum].Gun_DisplayName = GunRetail[GunNum].Gun_DisplayName;
		// BALLISTIC 
		GunNew[GunNum].Gun_NumBarrels = GunRetail[GunNum].Gun_NumBarrels;
		// Ballistic 
		GunNew[GunNum].Gun_Fire_Time = GunRetail[GunNum].Gun_Fire_Time;
		GunNew[GunNum].Gun_Alt_Fire_Time = GunRetail[GunNum].Gun_Alt_Fire_Time;
		GunNew[GunNum].Gun_NPC_Fire_Time = GunRetail[GunNum].Gun_NPC_Fire_Time;
		GunNew[GunNum].Gun_NPC_Alt_Fire_Time = GunRetail[GunNum].Gun_NPC_Alt_Fire_Time;
		
		GunNew[GunNum].Gun_Speed = GunRetail[GunNum].Gun_Speed;
		GunNew[GunNum].Gun_Alt_Speed = GunRetail[GunNum].Gun_Alt_Speed;
		GunNew[GunNum].Gun_Range = GunRetail[GunNum].Gun_Range;
		GunNew[GunNum].Gun_Alt_Range = GunRetail[GunNum].Gun_Alt_Range;
		GunNew[GunNum].Gun_Range = GunRetail[GunNum].Gun_Range;
		GunNew[GunNum].Gun_Alt_Range = GunRetail[GunNum].Gun_Alt_Range;
		GunNew[GunNum].Gun_Life = GunRetail[GunNum].Gun_Life;
		GunNew[GunNum].Gun_Alt_Life = GunRetail[GunNum].Gun_Alt_Life;
		// BRYAR BALISTIC 
		GunNew[GunNum].Gun_Charge_Unit = GunRetail[GunNum].Gun_Charge_Unit;
		GunNew[GunNum].Gun_Alt_Charge_Unit = GunRetail[GunNum].Gun_Alt_Charge_Unit;
		// BLASTER RIFLE
		GunNew[GunNum].Gun_Spread = GunRetail[GunNum].Gun_Spread;
		GunNew[GunNum].Gun_Alt_Spread = GunRetail[GunNum].Gun_Alt_Spread;
		// SNIPER RIFLE 
		GunNew[GunNum].Gun_ZoomFov = GunRetail[GunNum].Gun_ZoomFov;
		GunNew[GunNum].Gun_ZoomSoundStart = GunRetail[GunNum].Gun_ZoomSoundStart;
		GunNew[GunNum].Gun_ZoomSoundEnd = GunRetail[GunNum].Gun_ZoomSoundEnd;
		GunNew[GunNum].Gun_ZoomMask = GunRetail[GunNum].Gun_ZoomMask;
		GunNew[GunNum].Gun_ZoomRotateMask = GunRetail[GunNum].Gun_ZoomRotateMask;
		GunNew[GunNum].Gun_ZoomTick = GunRetail[GunNum].Gun_ZoomTick;
		GunNew[GunNum].Gun_ZoomCharge = GunRetail[GunNum].Gun_ZoomCharge;
		GunNew[GunNum].Gun_Crosshair = GunRetail[GunNum].Gun_Crosshair;
		// REPEATER 
		GunNew[GunNum].Gun_Gravity = GunRetail[GunNum].Gun_Gravity;
		GunNew[GunNum].Gun_Alt_Gravity = GunRetail[GunNum].Gun_Alt_Gravity;
		GunNew[GunNum].Gun_AntiGravity = GunRetail[GunNum].Gun_AntiGravity;
		GunNew[GunNum].Gun_Alt_AntiGravity = GunRetail[GunNum].Gun_Alt_AntiGravity;
		// FLECHETTE 
		GunNew[GunNum].Gun_Num_Shot = GunRetail[GunNum].Gun_Num_Shot;
		GunNew[GunNum].Gun_Alt_Num_Shot = GunRetail[GunNum].Gun_Alt_Num_Shot;
		GunNew[GunNum].Gun_Num_Bounce = GunRetail[GunNum].Gun_Num_Bounce;
		GunNew[GunNum].Gun_Alt_Num_Bounce = GunRetail[GunNum].Gun_Alt_Num_Bounce;
		GunNew[GunNum].Gun_Time_BombShot = GunRetail[GunNum].Gun_Time_BombShot;
		GunNew[GunNum].Gun_Alt_Time_BombShot = GunRetail[GunNum].Gun_Alt_Time_BombShot;
		// STUN BATON 
		GunNew[GunNum].Gun_ElectrocuteDuration = GunRetail[GunNum].Gun_ElectrocuteDuration;
		GunNew[GunNum].Gun_Alt_ElectrocuteDuration = GunRetail[GunNum].Gun_Alt_ElectrocuteDuration;
		GunNew[GunNum].Gun_StunTime = GunRetail[GunNum].Gun_StunTime;
		GunNew[GunNum].Gun_Alt_StunTime = GunRetail[GunNum].Gun_Alt_StunTime;
		// EXPLOSIVES 
		GunNew[GunNum].Gun_Time_Grenade_Time = GunRetail[GunNum].Gun_Time_Grenade_Time;
		GunNew[GunNum].Gun_Alt_Time_Grenade_Time = GunRetail[GunNum].Gun_Alt_Time_Grenade_Time;
		GunNew[GunNum].Gun_PoisonDamage = GunRetail[GunNum].Gun_PoisonDamage;
		GunNew[GunNum].Gun_Alt_PoisonDamage = GunRetail[GunNum].Gun_Alt_PoisonDamage;
		GunNew[GunNum].Gun_PoisonTime = GunRetail[GunNum].Gun_PoisonTime;
		GunNew[GunNum].Gun_Alt_PoisonTime = GunRetail[GunNum].Gun_Alt_PoisonTime;
		GunNew[GunNum].Gun_Hit_Animation = GunRetail[GunNum].Gun_Alt_Hit_Animation;
		GunNew[GunNum].Gun_Hit_Animation = GunRetail[GunNum].Gun_Alt_Hit_Animation;
		// Visual effects 
		GunNew[GunNum].Gun_Muzzle_Effect = GunRetail[GunNum].Gun_Muzzle_Effect;
		GunNew[GunNum].Gun_Alt_Muzzle_Effect = GunRetail[GunNum].Gun_Alt_Muzzle_Effect;
		GunNew[GunNum].Gun_Bullet_Effect = GunRetail[GunNum].Gun_Bullet_Effect;
		GunNew[GunNum].Gun_Alt_Bullet_Effect = GunRetail[GunNum].Gun_Alt_Bullet_Effect;
		GunNew[GunNum].Gun_Ray_Effect = GunRetail[GunNum].Gun_Ray_Effect;
		GunNew[GunNum].Gun_Alt_Ray_Effect = GunRetail[GunNum].Gun_Alt_Ray_Effect;
		GunNew[GunNum].Gun_WallImpact_Effect = GunRetail[GunNum].Gun_WallImpact_Effect;
		GunNew[GunNum].Gun_Alt_WallImpact_Effect = GunRetail[GunNum].Gun_Alt_WallImpact_Effect;
		GunNew[GunNum].Gun_FleshImpact_Effect = GunRetail[GunNum].Gun_FleshImpact_Effect;
		GunNew[GunNum].Gun_Alt_FleshImpact_Effect = GunRetail[GunNum].Gun_Alt_FleshImpact_Effect;
		GunNew[GunNum].Gun_Bounce_Effect = GunRetail[GunNum].Gun_Bounce_Effect;
		GunNew[GunNum].Gun_Alt_Bounce_Effect = GunRetail[GunNum].Gun_Alt_Bounce_Effect;
		GunNew[GunNum].Gun_Charge_Shader_Effect = GunRetail[GunNum].Gun_Charge_Shader_Effect;
		GunNew[GunNum].Gun_Alt_Charge_Shader_Effect = GunRetail[GunNum].Gun_Alt_Charge_Shader_Effect;
		GunNew[GunNum].Gun_Disgregate_Effect = GunRetail[GunNum].Gun_Disgregate_Effect;
		GunNew[GunNum].Gun_Alt_Disgregate_Effect = GunRetail[GunNum].Gun_Alt_Disgregate_Effect;
		// Damages 
		GunNew[GunNum].Gun_Damage = GunRetail[GunNum].Gun_Damage;
		GunNew[GunNum].Gun_AltDamage = GunRetail[GunNum].Gun_AltDamage;
		GunNew[GunNum].Gun_SplashDamage = GunRetail[GunNum].Gun_SplashDamage;
		GunNew[GunNum].Gun_AltSplashDamage = GunRetail[GunNum].Gun_AltSplashDamage;
		GunNew[GunNum].Gun_SplashRadius = GunRetail[GunNum].Gun_SplashRadius;
		GunNew[GunNum].Gun_AltSplashRadius = GunRetail[GunNum].Gun_AltSplashRadius;
		GunNew[GunNum].Gun_SplashKnockback = GunRetail[GunNum].Gun_SplashKnockback;
		GunNew[GunNum].Gun_AltSplashKnockback = GunRetail[GunNum].Gun_AltSplashKnockback;
		GunNew[GunNum].Gun_NPC_Damage_Easy = GunRetail[GunNum].Gun_NPC_Damage_Easy;
		GunNew[GunNum].Gun_NPC_Damage_Medium = GunRetail[GunNum].Gun_NPC_Damage_Medium;
		GunNew[GunNum].Gun_NPC_Damage_Hard = GunRetail[GunNum].Gun_NPC_Damage_Hard;
		GunNew[GunNum].Gun_Alt_NPC_Damage_Easy = GunRetail[GunNum].Gun_Alt_NPC_Damage_Easy;
		GunNew[GunNum].Gun_Alt_NPC_Damage_Medium = GunRetail[GunNum].Gun_Alt_NPC_Damage_Medium;
		GunNew[GunNum].Gun_Alt_NPC_Damage_Hard = GunRetail[GunNum].Gun_Alt_NPC_Damage_Hard;
		// Animations 
		//done in game (server-side code)
		GunNew[GunNum].GunMoveSpeedScale = GunRetail[GunNum].GunMoveSpeedScale;
		GunNew[GunNum].GunAnimSpeedScale = GunRetail[GunNum].GunAnimSpeedScale;	
		//Animation 
		GunNew[GunNum].Gun_Charger_Anim = GunRetail[GunNum].Gun_Charger_Anim;
		GunNew[GunNum].Gun_Alt_Charger_Anim = GunRetail[GunNum].Gun_Alt_Charger_Anim;
		GunNew[GunNum].Gun_ShootAnim = GunRetail[GunNum].Gun_ShootAnim;
		GunNew[GunNum].Gun_Alt_ShootAnim = GunRetail[GunNum].Gun_Alt_ShootAnim;
		GunNew[GunNum].Gun_ReadyAnim = GunRetail[GunNum].Gun_ReadyAnim;
		GunNew[GunNum].Gun_WalkAnim = GunRetail[GunNum].Gun_WalkAnim;
		GunNew[GunNum].Gun_RunAnim = GunRetail[GunNum].Gun_RunAnim;
		GunNew[GunNum].Gun_DropAnim = GunRetail[GunNum].Gun_DropAnim;
		GunNew[GunNum].Gun_MagicRestriction = GunRetail[GunNum].Gun_MagicRestriction;
		// Flags 
		GunNew[GunNum].gunFlags = GunRetail[GunNum].gunFlags;
		GunNew[GunNum].gunFlags2 = GunRetail[GunNum].gunFlags2;
		GunNew[GunNum].gunFlags3 = GunRetail[GunNum].gunFlags3;
		// other stuff
		GunNew[GunNum].Gun_Loop_Sound = GunRetail[GunNum].Gun_Loop_Sound;
		GunNew[GunNum].Gun_MOD = GunRetail[GunNum].Gun_MOD;
		GunNew[GunNum].Gun_Alt_MOD = GunRetail[GunNum].Gun_Alt_MOD;
		GunNew[GunNum].Gun_Poison_Effect = GunRetail[GunNum].Gun_Poison_Effect;
		GunNew[GunNum].Gun_Alt_Poison_Effect = GunRetail[GunNum].Gun_Alt_Poison_Effect;
		GunNew[GunNum].Gun_PoisonAnim = GunRetail[GunNum].Gun_PoisonAnim;
		GunNew[GunNum].Gun_Alt_PoisonAnim = GunRetail[GunNum].Gun_Alt_PoisonAnim;
		*/
	...
	if (iReadSize != iSize)
	{
		...
		{//
			...
			case INT_ID('G','C','L','I'):
				...
				break;
				if ( iSize == (int)(iReadSize+((sizeof(GunInfo_t)-sizeof(GunInfoRetail_t))*2)) )
				{
					gclient_t newClient;
					const int	preGunDataSize = ((intptr_t)&newClient.gs.gun[0]-(intptr_t)&newClient);
					memcpy( &newClient, pbData, preGunDataSize );
					SG_ConvertRetailGuninfoToNewGuninfo( ((void *)(&((gclient_t *)(pbData))->gs.gun[0])), &newClient.gs.gun[0] ); 
					memcpy( &newClient.gs.dualGuns, pbData+preGunDataSize+(sizeof(GunInfoRetail_t)*2), sizeof(newClient)-(preGunDataSize+(sizeof(GunInfo_t)*2)) );
					memcpy( pbData, &newClient, sizeof(gclient_t) );
				}
				else
				{//opps, not a gunInfo size mismatch, some other FUBAR-ness...
					G_Error(va("EvaluateFields(): variable-sized chunk '%s' without handler!",SG_GetChidText(ulChid)));
				}
				break;
