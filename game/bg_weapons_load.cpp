...
#include "weapons.h"
#include "bg_local.h"
#include "g_local.h" // for include DAMAGE stuff
#include "bg_damage.h"
...
static int GetDamageFlagFromString(const char* szDamageFlagString)
{
...
static void ParseWeaponStatsFlags ( weaponData_t *weaponData, const char *flagStr )
{
...
static void ParseDamage ( weaponFireModeStats_t *fireModeStats, cJSON *damageNode, qboolean secondary )
{
    if ( !damageNode )
    {
        return;
    ...
    else if ( cJSON_IsObject (damageNode) )
    {
		...
		{
			darea.damageFlags |= GetDamageFlagFromString(cJSON_ToStringOpt(cJSON_GetArrayItem(node, i), ""));
		}
		...
		else
		{
			...
			for (int i = 0; i < darea.numberDebuffs; i++)
			{
				...
				darea.debuffs[i].debuff = ResolveBuffName(cJSON_ToStringOpt(childNode, ""));
...
static void ParseWeaponFireMode ( weaponFireModeStats_t *fireModeStats, cJSON *fireModeNode )
{
    cJSON *node;
    const char *str = NULL;
...
//#ifdef _GAME
    ParseDamage (fireModeStats, node, qfalse);
    
    node = cJSON_GetObjectItem (fireModeNode, "secondarydamage");
    ParseDamage (fireModeStats, node, qtrue);
//#else
...
//#endif
	...
	fireModeStats->weaponMOD = GetMeansOfDamageIndex(str);
    
    node = cJSON_GetObjectItem (fireModeNode, "splashmeansofdeath");
    str = cJSON_ToStringOpt (node, "MOD_UNKNOWN");
	fireModeStats->weaponSplashMOD = GetMeansOfDamageIndex(str);
	...
	if (!fireModeStats->useQuantity) {
		...
		//QQQ WE NEED AGAIN TO INIT IT!!! fireModeStats->ammoBase = GetAmmo(cJSON_ToStringOpt(node, "AMMO_NONE"));

		node = cJSON_GetObjectItem(fireModeNode, "ammoDefault");
		// /// WE NEED AGAIN TO INIT!!! fireModeStats->ammoDefault = GetAmmo(cJSON_ToStringOpt(node, "AMMO_NONE"));
	}
}
...
//=========================================================
static void ParseWeaponStats ( weaponData_t *weaponData, cJSON *statsNode )
{
    cJSON *node;
    const char *flags[4];
    ...
    if ( node != NULL )
    {
        ...
        {
            ParseWeaponStatsFlags (weaponData, flags[i]);
        }
    }
...
static void ParseAnimationObject ( cJSON *animationNode, int *torsoAnimation, int *legsAnimation )
{
...
static void ParseWeaponPlayerAnimations ( weaponData_t *weaponData, cJSON *playerAnimNode )
{
    cJSON *node;
	...
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.ready.torsoAnim, &weaponData->anims.ready.legsAnim);
	}
    
    node = cJSON_GetObjectItem (playerAnimNode, "firing");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.firing.torsoAnim, &weaponData->anims.firing.legsAnim);
	}
    
    node = cJSON_GetObjectItem (playerAnimNode, "reload");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.reload.torsoAnim, &weaponData->anims.reload.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "backwardWalk");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.backwardWalk.torsoAnim, &weaponData->anims.backwardWalk.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "forwardWalk");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.forwardWalk.torsoAnim, &weaponData->anims.forwardWalk.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "crouchWalk");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.crouchWalk.torsoAnim, &weaponData->anims.crouchWalk.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "crouchWalkBack");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.crouchWalkBack.torsoAnim, &weaponData->anims.crouchWalkBack.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "jump");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.jump.torsoAnim, &weaponData->anims.jump.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "land");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.land.torsoAnim, &weaponData->anims.land.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "run");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.run.torsoAnim, &weaponData->anims.run.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "sprint");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.sprint.torsoAnim, &weaponData->anims.sprint.legsAnim);
	}
	...
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.sights.torsoAnim, &weaponData->anims.sights.legsAnim);
	}

	node = cJSON_GetObjectItem (playerAnimNode, "sightsFiring");
	if(node)
	{
		ParseAnimationObject (node, &weaponData->anims.sightsFiring.torsoAnim, &weaponData->anims.sightsFiring.legsAnim);
	}
}
...
const char *CG_GetStringEdString2(char *refName)
{
	...
	{
		// Reference it up!
		gi.SE_GetStringTextString(refName+1, text[index], sizeof(text[0]));
	}
	else
	{
...
static void ParseVisualsFireMode ( weaponVisualFireMode_t *fireMode, cJSON *fireModeNode, int numFireModes )
{
    cJSON *node = NULL;
    cJSON *child = NULL;
...
static void ParseVisuals ( weaponData_t *weaponData, cJSON *visualsNode )
{
	...
	// EHI I WANT TO GET PRIMARY AND SECONDARY FIRE! -,- QQQ 
    node = cJSON_GetObjectItem (visualsNode, "primary");
//    ParseVisualsFireMode (&weaponVisuals->primary, node);
    
    node = cJSON_GetObjectItem (visualsNode, "secondary");
 //   ParseVisualsFireMode (&weaponVisuals->secondary, node);*/
	...
	for ( int i = 0; i < weaponData->numFiringModes; i++ )
	{
		...
		if(node)
		{
			ParseVisualsFireMode(&weaponVisuals->visualFireModes[i], node, weaponData->numFiringModes);
		}
	}
}

static void ParseWPNSaberData ( weaponData_t *wp, cJSON *json )
{
	...
	// WE NOT CARE ABOUT SABER STUFF, RIGHT? 
}

extern stringID_table_t WPTable[]; // From bg_saga.c
extern void AddWeaponData ( weaponData_t *weaponData ); 
#define MAX_WEAPON_FILE_LENGTH (16384) // 16kb should be enough, 4kb apparently wasn't!
static qboolean ParseWeaponFile ( const char *weaponFilePath )
{
    cJSON *json = NULL;
    cJSON *jsonNode = NULL;
    ...
    fileHandle_t f;
	int fileLen = gi.FS_FOpenFile (weaponFilePath, &f, FS_READ);
	...
	gi.FS_Read (&weaponFileData, fileLen, f);
	...
	for(i = 0; i < MAX_FIREMODES; i++)
	{
		...
		if(jsonNode != NULL)
		{
			ParseWeaponFireMode (&weaponData.firemodes[i], jsonNode);
			weaponData.numFiringModes++;
		}
	}
    
    jsonNode = cJSON_GetObjectItem (json, "playeranims");
    ParseWeaponPlayerAnimations (&weaponData, jsonNode);
...
//#ifdef _CGAME
    ...
    str = cJSON_ToString (jsonNode);
    Q_strncpyz (weaponData.visuals.WPNdescription, str, sizeof (weaponData.visuals.WPNdescription));
//#endif
    jsonNode = cJSON_GetObjectItem (json, "visual");
    ParseVisuals (&weaponData, jsonNode);
...
static qboolean LoadWeaponCacheFile ( weaponData_t *weaponDataTable, int *numLoadedWeapons, int *numWeapons )
{
	...
	fileLen = gi.FS_FOpenFile ("ext_data/weapons/weapcache.bin", &f, FS_READ);
    if ( fileLen == -1 || !f )
    {
        return qfalse;
    }
	...
	gi.FS_Read ((void *)buffer, fileLen, f);
	gi.FS_FCloseFile (f);
    buffer[fileLen] = '\0';
...
static void WriteWeaponCacheFile ( weaponData_t *weaponDataTable, unsigned int *numLoadedWeapons )// was int numloadedweapons QQQ 
{
    fileHandle_t f;
    char *buffer;
    char *p;
	...
	gi.FS_FOpenFile ("ext_data/weapons/weapcache.bin", &f, FS_WRITE);
    if ( !f )
    {
        return;
    }
    ...
    gi.FS_Write ((void *)buffer, bufferSize, f);
	gi.FS_FCloseFile (f);
...
qboolean LoadWeapons ( weaponData_t *weaponDataTable, unsigned int *numLoadedWeapons, unsigned int *numWeapons )
{
    int i;
    char weaponFiles[8192];
	int numFiles = gi.FS_GetFileList ("ext_data/weapons", ".wpn", weaponFiles, sizeof (weaponFiles));
    const char *weaponFile = weaponFiles;
    int successful = 0;
    int failed = 0;
    
	int t = gi.Milliseconds();
    ...
    //if ( !LoadWeaponCacheFile (weaponDataTable, numLoadedWeapons, numWeapons) )
    {
        ...
        {
            if ( ParseWeaponFile (va ("ext_data/weapons/%s", weaponFile)) )
            {
                successful++;
            }
            else
...
void InitWeaponG2Instances(void) 
{
	unsigned int i = 0;
	unsigned int j;
	memset(g2WeaponInstances, 0, sizeof(g2WeaponInstances));
	// CANNOT COMPILE FOR MOMEMT. OBJ FUCKED OUT INTO "NUMBER OF WEAPON VARIATIONS... 
	/*
	for ( i = 0; i <= LAST_USEABLE_WEAPON; i++ )
	{
	    unsigned int numVariations = NumberOfWeaponVariations (i);
	    for ( j = 0; j < numVariations; j++ )
	    {
	        const weaponData_t *weaponData = GetWeaponData (i, j);
			const int id = GetWeaponIndex(i, j);
				...
				weaponData->visuals.world_model[0])		// hard to code with all this background noise going on --eez
			{
				...
				// SIGH WE NEED TO MAKE OUR OWN INIT 
				// FUNCTIONS FOR MAKE THAT DOABLE... 
				gi.G2API_JKGInitGhoul2Model (&g2WeaponInstances[id].ghoul2, weaponData->visuals.world_model, 0, 0, 0, 0, 0);
	        
				ghoul2 = g2WeaponInstances[id].ghoul2;
	        
				if ( gi.G2_HaveWeGhoul2Models (ghoul2) )
				{
					gi.G2API_SetBoltInfo (ghoul2, 0, 0);
					gi.G2API_JKGAddBolt (ghoul2, 0, i == WP_SABER ? "*blade1" : "*flash");
...
void ShutdownWeaponG2Instances(void) 
{
	
	/*unsigned int i = 0; // OBJ FUCKED OUT INT "NUMBEROFWEAPONVARIATION. 
	unsigned int j;
	unsigned int id = 0;
	
	for ( i = 0; i <= LAST_USEABLE_WEAPON; i++ )
	{
		unsigned int numVariations = NumberOfWeaponVariations (i); // QQQQ WE NOT KNOW
		// HOW TO FIX THE UNSIGNED INT. 
	    for ( j = 0; j < numVariations; j++ )
	    {
			if(g2WeaponInstances[id].ghoul2 && gi.G2_HaveWeGhoul2Models(g2WeaponInstances[id].ghoul2)) 
			{
				gi.G2API_JKGCleanGhoul2Models(&g2WeaponInstances[id].ghoul2);
...
void JKG_GetWeaponGhoul2 ( int weaponNum, int weaponVariation )
{
	const int id = GetWeaponIndex(weaponNum, weaponVariation);
