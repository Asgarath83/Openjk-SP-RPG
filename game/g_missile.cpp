...
//-------------------------------------------------------------------------
void G_MissileBounceImpactEffect( gentity_t *ent, vec3_t org, vec3_t dir, qboolean hitWorld )
{
	//FIXME: have an EV_BOUNCE_MISSILE event that checks the s.weapon and does the appropriate effect
	//
	
	if ( ent->owner->client->gs.gun->gunFlags&GFL_BOUNCESHOOT )
		{ 
			if ( ent->owner->client->gs.gun->Gun_WallImpact_Effect )
			{
				G_PlayEffect( ent->owner->client->gs.gun->Gun_WallImpact_Effect, org, dir );
			}
		}
		else if ( ent->owner->client->gs.gun->gunFlags&GFL_ALT_BOUNCESHOOT )
		{
			if ( ent->owner->client->gs.gun->Gun_Alt_WallImpact_Effect )
			{
				G_PlayEffect( ent->owner->client->gs.gun->Gun_Alt_WallImpact_Effect, org, dir );
			}
		}
	// NOT DO IF FLAG IS TURNED OFF!
}
...
void G_MissileImpacted( gentity_t *ent, gentity_t *other, vec3_t impactPos, vec3_t normal, int hitLoc=HL_NONE )
{
	...
	if ( other->takedamage ) 
	{
		...
		if ( ent->damage ) 
		{
			...
			if ( ent->s.weapon == WP_DEMP2 || ent->s.weapon == WP_BOWCASTER )
...
void G_MissileImpact( gentity_t *ent, trace_t *trace, int hitLoc=HL_NONE ) 
{
	...
	{
		// BOUNCE DISABILITATO PER TUTTE LE ARMI TK E DA SPARO
		bounce = qfalse;
...
extern bool WP_DoingMoronicForcedAnimationForForcePowers(gentity_t *ent);
	...
	if ( other->contents & CONTENTS_LIGHTSABER )
	{
		...
		if ( ( g_spskill->integer <= 0 && ent->s.weapon != WP_BLASTER_PISTOL && ent->s.weapon != WP_BOWCASTER && ent->s.weapon != WP_BLASTER && ent->s.weapon != WP_REPEATER && ent->s.weapon != WP_FLECHETTE && ent->s.weapon != WP_DEMP2 && ent->s.weapon != WP_CONCUSSION && ent->s.weapon != WP_JAWA && ent->s.weapon != WP_ATST_MAIN && ent->s.weapon != WP_EMPLACED_GUN && ent->s.weapon != WP_TURRET && ent->s.weapon != WP_POISON && ent->s.weapon != WP_TUSKEN_RIFLE && ent->s.weapon != WP_BRYAR_PISTOL && ent->s.weapon != WP_ROCKET_LAUNCHER //on easy, it reflects all shots
				|| (g_spskill->integer == 1 && ent->s.weapon != WP_BLASTER_PISTOL && ent->s.weapon != WP_BOWCASTER && ent->s.weapon != WP_BLASTER && ent->s.weapon != WP_REPEATER && ent->s.weapon != WP_FLECHETTE && ent->s.weapon != WP_DEMP2 && ent->s.weapon != WP_CONCUSSION && ent->s.weapon != WP_JAWA && ent->s.weapon != WP_ATST_MAIN && ent->s.weapon != WP_EMPLACED_GUN && ent->s.weapon != WP_TURRET && ent->s.weapon != WP_POISON && ent->s.weapon != WP_TUSKEN_RIFLE && ent->s.weapon != WP_BRYAR_PISTOL && ent->s.weapon != WP_ROCKET_LAUNCHER )//on medium it won't reflect flechette or demp shots
				|| (g_spskill->integer >= 2 && ent->s.weapon != WP_BLASTER_PISTOL && ent->s.weapon != WP_BOWCASTER && ent->s.weapon != WP_BLASTER && ent->s.weapon != WP_REPEATER && ent->s.weapon != WP_FLECHETTE && ent->s.weapon != WP_DEMP2 && ent->s.weapon != WP_CONCUSSION && ent->s.weapon != WP_JAWA && ent->s.weapon != WP_ATST_MAIN && ent->s.weapon != WP_EMPLACED_GUN && ent->s.weapon != WP_TURRET && ent->s.weapon != WP_POISON && ent->s.weapon != WP_TUSKEN_RIFLE && ent->s.weapon != WP_BRYAR_PISTOL && ent->s.weapon != WP_ROCKET_LAUNCHER )//on hard it won't reflect flechette, demp, repeater or bowcaster shots
			 )
