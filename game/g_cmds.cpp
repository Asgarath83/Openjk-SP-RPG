...
void G_Give( gentity_t *ent, const char *name, const char *args, int argc )
{
	...
	{
		//ent->client->ps.stats[STAT_WEAPONS] = (unsigned)(1 << WP_POISON) - ( 1 << WP_NONE ); // NOTE: this wasn't giving the last weapon in the list
		for ( int i = 0; i < WP_NUM_WEAPONS; i++ )
		{
			ent->client->ps.weapons[i] = 1;
		}
...
extern gentity_t *G_DropPistolItem( const char *PistolType, vec3_t PistolPos, vec3_t PistolVel, vec3_t PistolAngles, gentity_t *copyPistol );
...
// Pistol Drop
void Cmd_PistolDrop_f( gentity_t *ent, int PistolNum )
{
	if ( PistolNum < 0 )
	{
		return;
	...
	if ( ent->weaponModel[PistolNum] <= 0 )
	{
		return;
	}
	...
	if ( !g_gunSwapPistols->integer )
	{
		return;
	}
	
	if ( !ent->client->gs.gun[PistolNum].Gun_Name
		|| !ent->client->gs.gun[PistolNum].Gun_Name[0] )
	{
		return;
	}
	...
	//turn it into a pick-uppable item!
	if ( G_DropPistolItem( ent->client->gs.gun[PistolNum].Gun_Name, 
			(PistolNum==0?ent->client->renderInfo.handRPoint:ent->client->renderInfo.handLPoint), 
			ent->client->ps.velocity, 
			ent->currentAngles ) 
		!= NULL )
	{//dropped it
		WP_RemoveGun( ent, PistolNum );
	}

	if ( ent->weaponModel[0] <= 0
		&& ent->weaponModel[1] <= 0 )
	{//no pistols left!
		//remove pistols from inventory
		ent->client->ps.weapons[WP_BLASTER_PISTOL] = 0;
		//change weapons
		if ( ent->s.number < MAX_CLIENTS )
		{//player
			CG_ChangeWeapon( WP_NONE );
...
//		G_Taunt( ent );
	...
	*/
	...
	else if ( Q_stricmp( cmd, "droppistol" ) == 0 )
	{
		const char *cmd2 = gi.argv(1);
		int	saberNum = 2;//by default, drop both
		if ( cmd2 && cmd2[0] )
		...
		if ( saberNum > 1 )
		{//drop both
			Cmd_PistolDrop_f( ent, 1 );
			Cmd_PistolDrop_f( ent, 0 );
		}
		else
		{//drop either left or right
			Cmd_PistolDrop_f( ent, saberNum );
		}
	}
	else
	{
