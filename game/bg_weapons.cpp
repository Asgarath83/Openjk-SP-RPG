...
qboolean GetWeaponByIndex ( int index, int *weapon, int *variation ) { 
...
int GetWeaponIndexFromClass ( int weapon, int variation )
{
...
weaponData_t *GetWeaponByClassName ( const char *className )
{
...
int GetWeaponIndex ( unsigned int weapon, unsigned int variation )
{
    int i;
    for ( i = 0; i < numLoadedWeapons; i++ )
    {
...
qboolean WeaponVariationExists ( unsigned int weaponId, unsigned int variation )
{
    return GetWeaponIndex (weaponId, variation) != numLoadedWeapons;
}

qboolean WeaponCanUseSpecialAmmo (weaponData_t* wp) {
...
{
    //return ammoTable[ammoIndex].ammoMax; FOR MOMENT WE CANNOT USE -.- WE NEED TO FIX AMMO BEFORE THAT! 
	return NULL; // temp fix 
}

// Just in case we need to initialize the struct any other way,
// it's easier to have it's own function.
void InitializeWeaponData ( weaponData_t *weaponData )
{
...
/*	weaponData->anims.ready.torsoAnim = BOTH_STAND1;
	...
	// WE FIX INTO ANOTHER MOMENT, OKAY? 

}

void AddWeaponData ( weaponData_t *weaponData )
{
...
//extern void InitializeAmmo(void ); QQQ FUCKED OUT FOR NOW... 
extern qboolean LoadWeapons ( weaponData_t *weaponDataTable, unsigned int *numLoadedWeapons, unsigned int *numWeapons ); 
void InitializeWeapons ( void )
{
	gi.Printf ( "LOADING WEAPONS LIBRARY!\n");
    ...
    if ( !LoadWeapons (weaponDataTable, &numLoadedWeapons, numWeapons) )
    {
...
qboolean DumpWeaponList ( const char *filename )
{
    char buffer[8192] = { 0 };
	...
	gi.FS_FOpenFile (filename, &f, FS_WRITE);
    if ( f )
    {
		gi.FS_Write (buffer, strlen (buffer), f);
		gi.FS_FCloseFile (f);
