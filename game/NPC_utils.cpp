...
qboolean JET_Flying( gentity_t *self )
{
	...
	else if ( self->client->NPC_class == CLASS_BIRD ||
		 self->client->NPC_class == CLASS_POLTER ||
		 self->client->NPC_class == CLASS_REAPER || 
		 self->client->NPC_class == CLASS_ARIEL ||
		 self->client->NPC_class == CLASS_VAMPIRE_GHOST || 
		 self->client->NPC_class == CLASS_BATFORM ||
		 self->client->NPC_class == CLASS_BATKAIN ||
		 self->client->NPC_class == CLASS_BATKAIN2 ||
		 self->client->NPC_class == CLASS_BATKAIN3 ||
		 self->client->NPC_class == CLASS_BATVORADOR ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_QUEEN ||
		 self->client->NPC_class == CLASS_BATNECROMANCER ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_WATER ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_ELF ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_ELF2 ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_ELF3 ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_FROST || 
		  self->client->NPC_class == CLASS_HELLBAT ||
		 self->client->NPC_class == CLASS_BATVAMPIRE_FEAR ||
		  self->client->NPC_class == CLASS_BATVAMPIRE_FEAR2 ||
		  self->client->NPC_class == CLASS_JANOS ||
		  self->client->NPC_class == CLASS_KAIN3 )
	{
		return (NPC_Flying(self));
	}
	else
	{
		return qfalse;
