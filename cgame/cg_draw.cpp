...
static void CG_DrawFighterHud( centity_t *cent )
{
	int xPos,yPos,width,height;
	vec4_t	color;
	qhandle_t	background;
...
*/
	...
	// in a Fighter?
	else if ( cent->gent->client->NPC_class == CLASS_FIGHTER ) 
	{
		CG_DrawFighterHud(cent);
		return qfalse; // drew this hud, so don't draw the player one
	}
	// In an ATST
...
static void CG_Draw2D( void ) 
{
	...
	{//force sight is on
		...
		// FORCE SIGHT SHADER CHANGED FOR CLASSES 
		// - DOMANI CASE SWITCH FORCE SIGHT.

		switch ( cent->gent->client->NPC_class )
			{
				// Piante e animali usano ultrasuoni
				case CLASS_IVY:
				...
				{// ECHO SIGHT FOR TURELIM 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jecho" ));
					CG_DrawHealthBars();
				}
				break;
				// Umani ed Elfi usano visione glifica sacra per sgamare i vampiri
				// elfi 
				...
				// armature stregate
				case CLASS_MARK1:			// armature sarafan 
				// costrutti
				...
				{ // Visione Glifica 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense4" ));
					CG_DrawHealthBars();
				}
				break; 
	            // Tiraza - Lo Sguardo: questo incantamento 
				// permette di scrutare il Regno Spettrale da quello Materiale per individuare la presenza di anime, creature, portali, variazioni ambientali, ecc.
				// (Forse Sense del Mietitore)
				// Usato da Mietitori, Fantasmi, Non morti. 
				...
				case CLASS_MORGANKATARN: // Abdiel e Syl con radianza
				...
				case CLASS_REBEL:			// Naeryan con nebbia 
				case CLASS_ROCKETTROOPER:	// Naeryan con tempesta 
				...
				case CLASS_LANDO:	// Asgarath con astrale
				...
				{ // Light Spell 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense2" ));
					CG_DrawHealthBars();
				}
				break;
				// MIST FORM 
				// la forma a nebbia dei vampiri NON ha visioni speciali 
				...
				break;
				// Gli Elementali hanno vari tipi di percezione... 
				// Ombre e raggianti della Tenebra usano la visione oscura
				...
				{ // Dark vision 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense_dark" ));
					CG_DrawHealthBars();
				}
				break;
				// Gli elementali di fuoco usano la vista termica a infrarossi
				...
				{ 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense_fire" ));
					CG_DrawHealthBars();
				}
				break;
				// Gli elementali di Luce usano il Light dei Vampiri 
				...
				break;
					// elementali di suono e metallo usan gli echi
				...
				break;
				// Gli elementali d'Aria usano la visione del vento
				...
				{// ECHO SIGHT FOR TURELIM 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jecho2" ));
					CG_DrawHealthBars();
				}
				break;
				// Gli elementali d'Acqua usano blur vision
				...
				{// ECHO SIGHT FOR TURELIM 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jecho3" ));
					CG_DrawHealthBars();
				}
				break;
				// Gli elementali di Terra usano visione sismica infrasonica
				...
				{ // Light Spell 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jecho4" ));
					CG_DrawHealthBars();
				}
				break;
				// Visione dei fantasmi: funziona al contrario: scruta il Regno Materiale
				// dal Regno Spettrale
			   ...
			   // Golem del Sacro 
				...
				// Cerchio dei Nove, ormai sono fantasmi  
				...
				{
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense_ghost" ));
					CG_DrawHealthBars();
	 			}
				break; 
				// NON MORTI - percezione oscura 
				case CLASS_NECRO: 
				...
				{
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense_necro" ));
					CG_DrawHealthBars();
	 			}
				break; 
				// Gli Hylden hanno una visione peculiare data dalle loro creste craniali
			   ...
			   case CLASS_HYLDEN_MADNESS: // maestro della Follia
				...
				{// HYLDEN CRANIAL SIGHT 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense5" ));
					CG_DrawHealthBars();
				}
				break;
					
				// Visione Infernale, usata dai demoni in generis 
				// armature stregate usano la visione infernale
				...
				{// ELDER GOD EVIL SIGHT 
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense6" ));
					CG_DrawHealthBars();
	 			}	
				break; 
				case CLASS_FIGHTER: // starships should have a radar... 
				{
					CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jradar" ));
					CG_DrawHealthBars();
	 			}	
				break;
			default:
		...
		if ( cent->gent->client->NPC_class == CLASS_JEDI || cent->gent->client->NPC_class == CLASS_SABOTEUR || 
			cent->gent->client->NPC_class == CLASS_BESPIN_COP || cent->gent->client->NPC_class == CLASS_SHADOWTROOPER || 
			cent->gent->client->NPC_class == CLASS_ASSASSIN_DROID || cent->gent->client->NPC_class == CLASS_MORGANKATARN || 
			cent->gent->client->NPC_class == CLASS_REBEL || cent->gent->client->NPC_class == CLASS_ROCKETTROOPER || 
			cent->gent->client->NPC_class == CLASS_SWAMP || cent->gent->client->NPC_class == CLASS_BARTENDER || 
			cent->gent->client->NPC_class == CLASS_WEEQUAY || cent->gent->client->NPC_class == CLASS_UGNAUGHT || 
			cent->gent->client->NPC_class == CLASS_LIZARD || cent->gent->client->NPC_class == CLASS_REELO || 
			cent->gent->client->NPC_class == CLASS_COMMANDO || cent->gent->client->NPC_class == CLASS_MURJJ || 
			cent->gent->client->NPC_class == CLASS_STORMTROOPER || cent->gent->client->NPC_class == CLASS_LUKE || 
			cent->gent->client->NPC_class == CLASS_TRANDOSHAN || cent->gent->client->NPC_class == CLASS_LANDO ||
			cent->gent->client->NPC_class == CLASS_JAWA || cent->gent->client->NPC_class == CLASS_RANCOR ||
			cent->gent->client->NPC_class == CLASS_BOBAFETT || 
			cent->gent->client->NPC_class == CLASS_REAVER || cent->gent->client->NPC_class == CLASS_DARKREAVER || 
			cent->gent->client->NPC_class == CLASS_TANKREAVER || cent->gent->client->NPC_class == CLASS_VOID || 
			cent->gent->client->NPC_class == CLASS_LIGHTREAVER || cent->gent->client->NPC_class == CLASS_AIRREAVER || 
			cent->gent->client->NPC_class == CLASS_SPIDERREAVER || cent->gent->client->NPC_class == CLASS_SPIDERREAVER2 || 
			cent->gent->client->NPC_class == CLASS_SPIDERREAVER3 || cent->gent->client->NPC_class == CLASS_EARTHREAVER || 
			cent->gent->client->NPC_class == CLASS_ICEREAVER || cent->gent->client->NPC_class == CLASS_MENTALIST || 
			cent->gent->client->NPC_class == CLASS_CRYSTAL || cent->gent->client->NPC_class == CLASS_GHOST || 
			cent->gent->client->NPC_class == CLASS_SLUAGH || cent->gent->client->NPC_class == CLASS_POLTER || 
			cent->gent->client->NPC_class == CLASS_HOLY || cent->gent->client->NPC_class == CLASS_RAZIEL || 
			cent->gent->client->NPC_class == CLASS_ARIEL || 
			cent->gent->client->NPC_class == CLASS_DRUIDREAVER || cent->gent->client->NPC_class == CLASS_AIR || 
			cent->gent->client->NPC_class == CLASS_THRALL || cent->gent->client->NPC_class == CLASS_NECRO ||
			cent->gent->client->NPC_class == CLASS_GOLEM_ICE || cent->gent->client->NPC_class == CLASS_GOLEM_DARK ||
			cent->gent->client->NPC_class == CLASS_GOLEM_SMOKE || cent->gent->client->NPC_class == CLASS_GOLEM_VOID ||
			cent->gent->client->NPC_class == CLASS_GOLEM_AIR || cent->gent->client->NPC_class == CLASS_GOLEM_HOLY || 
			cent->gent->client->NPC_class == CLASS_GOLEM_NECRO || cent->gent->client->NPC_class == CLASS_GOLEM_WATER ||
			cent->gent->client->NPC_class == CLASS_GOLEM_METAL )
		
		// SIGHT FOR ALL VAMPIRES CLASS
		else if ( cent->gent->client->NPC_class == CLASS_REBORN || cent->gent->client->NPC_class == CLASS_ALORA || 
			cent->gent->client->NPC_class == CLASS_WAMPA || cent->gent->client->NPC_class == CLASS_KYLE || 
			cent->gent->client->NPC_class == CLASS_NOGHRI || cent->gent->client->NPC_class == CLASS_SWAMPTROOPER || 
			cent->gent->client->NPC_class == CLASS_MONMOTHA || cent->gent->client->NPC_class == CLASS_MINEMONSTER ||
			cent->gent->client->NPC_class == CLASS_GLIDER || cent->gent->client->NPC_class == CLASS_FLIER2 ||
			cent->gent->client->NPC_class == CLASS_FISH || cent->gent->client->NPC_class == CLASS_PLAYER ||
			cent->gent->client->NPC_class == CLASS_PROTOCOL || cent->gent->client->NPC_class == CLASS_SABER_DROID ||
			cent->gent->client->NPC_class == CLASS_VAMPIRE ||// cent->gent->client->NPC_class == CLASS_KAIN || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_DEMON || cent->gent->client->NPC_class == CLASS_VAMPIRE_GHOST || 
			...
			cent->gent->client->NPC_class == CLASS_NECROMANCER || cent->gent->client->NPC_class == CLASS_VAMPIRE_QUEEN || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_FROST || 	cent->gent->client->NPC_class == CLASS_VAMPIRE_DARK || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_ARCHER || cent->gent->client->NPC_class == CLASS_UMAH || 
			...
			cent->gent->client->NPC_class == CLASS_VAMPIRE_LYCAN || cent->gent->client->NPC_class == CLASS_VAMPIRE_SOUND ||
			cent->gent->client->NPC_class == CLASS_VAMPIRE_WATER || cent->gent->client->NPC_class == CLASS_HALFVAMPIRE || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_BOUNTY || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_ELF || cent->gent->client->NPC_class == CLASS_VAMPIRE_ELF2 || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_ELF3 || cent->gent->client->NPC_class == CLASS_VAMPIRE_ZOMBIE || 
			cent->gent->client->NPC_class == CLASS_VAMPIRE_SARAFAN || 	cent->gent->client->NPC_class == CLASS_VAMPIRE_FEAR || 
			...
			cent->gent->client->NPC_class == CLASS_GOLEM_GOLD ) 
		{
		...
		else if ( cent->gent->client->NPC_class == CLASS_GRAN || cent->gent->client->NPC_class == CLASS_CLAW || 
			cent->gent->client->NPC_class == CLASS_HAZARD_TROOPER || cent->gent->client->NPC_class == CLASS_VEHICLE ||
			cent->gent->client->NPC_class == CLASS_GONK || cent->gent->client->NPC_class == CLASS_GALAKMECH ||
			cent->gent->client->NPC_class == CLASS_HOWLER || cent->gent->client->NPC_class == CLASS_INTERROGATOR ||
			cent->gent->client->NPC_class == CLASS_TURELIM || cent->gent->client->NPC_class == CLASS_FIREREAVER || 
			cent->gent->client->NPC_class == CLASS_FIRE || cent->gent->client->NPC_class == CLASS_IVY ||
			cent->gent->client->NPC_class == CLASS_EARTH || cent->gent->client->NPC_class == CLASS_CRYSTAL ||
			cent->gent->client->NPC_class == CLASS_SONIC || cent->gent->client->NPC_class == CLASS_POISON ||
			cent->gent->client->NPC_class == CLASS_SHARK || cent->gent->client->NPC_class == CLASS_BEAST ||
			cent->gent->client->NPC_class == CLASS_LYCAN || cent->gent->client->NPC_class == CLASS_MUTANT ||
			cent->gent->client->NPC_class == CLASS_BIRD ||
			cent->gent->client->NPC_class == CLASS_DARK || cent->gent->client->NPC_class == CLASS_WATER ||
			...
			cent->gent->client->NPC_class == CLASS_DEMON_POISON || // Shade
			cent->gent->client->NPC_class == CLASS_GOLEM )
		
		else if ( cent->gent->client->NPC_class == CLASS_JAN || cent->gent->client->NPC_class == CLASS_IMPWORKER || 
			cent->gent->client->NPC_class == CLASS_RODIAN || cent->gent->client->NPC_class == CLASS_PRISONER || 
			cent->gent->client->NPC_class == CLASS_IMPERIAL || cent->gent->client->NPC_class == CLASS_WIZARD || 
			cent->gent->client->NPC_class == CLASS_GALAK ||
			cent->gent->client->NPC_class == CLASS_MARK1 || cent->gent->client->NPC_class == CLASS_MARK2 ||
			cent->gent->client->NPC_class == CLASS_HUNTER || cent->gent->client->NPC_class == CLASS_SCOUT || 
			cent->gent->client->NPC_class == CLASS_ALCHEMIST || cent->gent->client->NPC_class == CLASS_PROPHET || 
			cent->gent->client->NPC_class == CLASS_CENOBITA || cent->gent->client->NPC_class == CLASS_ARCHER || 
			cent->gent->client->NPC_class == CLASS_WARRIOR || cent->gent->client->NPC_class == CLASS_SARAFAN || 
			cent->gent->client->NPC_class == CLASS_DRUID )
		{// GLYPHIC VISION FOR SARAFANS 
			CG_DrawPic( 0, 0, 640, 480, cgi_R_RegisterShader( "gfx/2d/jsense4" ));
			CG_DrawHealthBars();
	 	}
			...
			cent->gent->client->NPC_class == CLASS_HYLDEN || cent->gent->client->NPC_class == CLASS_HYLDENLORD || 
		...
		else if ( cent->gent->client->NPC_class == CLASS_SENTRY || cent->gent->client->NPC_class == CLASS_ATST || 
			cent->gent->client->NPC_class == CLASS_SAND_CREATURE || cent->gent->client->NPC_class == CLASS_MOUSE ||
			cent->gent->client->NPC_class == CLASS_TENTACLE || cent->gent->client->NPC_class == CLASS_ELDERGOD || 
			cent->gent->client->NPC_class == CLASS_DREADNOUGHT || cent->gent->client->NPC_class == CLASS_REAPER ||
