...
#ifndef __Q_SHARED_H
#define __Q_SHARED_H
#define NO_XINPUT // FOR SHUT UP DEBUGGER CRAPS! Ty to eez 
...
typedef struct cvar_s {
	...
	int			hashIndex;
	// NEW ADDING FOR HP CODE 
	//struct cvar_s *g_handicap_matchNPChp; 
...
#define MAX_FX				296//296 is the maximum value!//256//128
...
typedef enum
{
	...
	FP_STUN,//istant
	FP_HATE,//istant
	FP_CONTROLMIND,//istant
	FP_FREEZE,//istant
	FP_HEALOTHER,//istant
	FP_WRACK,//istant
	FP_FORCEGLYPH,//force repulse code
	FP_STONEGLYPH,// glifo pietra, reaver terra, polvere e pietra
	FP_SOUNDGLYPH,// glifo suono, reaver suono, reaver astrale / teletrasporto dimensionale
	FP_WATERGLYPH, // glifo acqua, reaver d'acqua, gelo, cristallo
	FP_FIREGLYPH, // atk aoe glifo fuoco, reaver fuoco, calore, magma  
	FP_SUNGLYPH, // atk aoe glifo luce, reaver luce, fulmine, radianza, energia
	FP_WINDGLYPH, // Force WIND atk aoe vento, nebbia, tempesta. 
	FP_HOLYGLYPH, // atk aoe di spirito, sacro, vita 
	FP_DARKGLYPH, // reaver tenebra, fumo, vuoto. la reaver di vuoto deve aver knockback potente come l'astrale. 
	FP_NECROGLYPH, // reaver vampirica, spettrale e di morte. 
	// NEW ENTRY FOR ASTROFANTASY 
	FP_ACIDGLYPH, // for atk of Mud, Poison and States reaver
	FP_WOLF,// transforminto a feral creature 
	FP_BAT, // tramuta in creautra alata. 
	FP_MIST, // tramuta in gas o permetti di volare schermato\ proiezione astrale etc. 
	FP_INSANE, // atk aoe Mente, Panico , Conflitto 
	FP_TEAMHEAL, 
	FP_BUFF, 
	FP_VALOR, 
	FP_CURSE, // DeadlySight muovilo qua come codice. 
	FP_BOLT, 
	FP_SALT,
	FP_AGG,
	FP_DARK,
...
#define	MAX_WEAPONS				1024 // I NEED!! 
#define MAX_WEAPONBITS			1 + (MAX_WEAPONS - 1)/32
#define MAX_ARMOR				16 // JKG! 
#define MAX_AMMO				10 // Adding more cause CRASH OF "GIVE ALL" CHEAT! 		
#define MAX_INVENTORY			15 //15	// See INV_MAX
...
{
	WNONE, // free slot now use . marker -
	STICK, // Tusken staff like weapon slots. for smugglers. 
	BOW,	 //  - bow slot, bryar pistol slot.  
	WOOKIECASTER,	// heavy bow categories, - bowcaster slot. Sun - Shock weapons  
	STEAMPUNK, // Steampunk shoot weapons  - Emplaced Gun Slot 
	COLUBRINE, // ancient cannon weapon or weapons that shoot as Demp2. Holy magic
	BATON, // stun baton slots.  -
	PISTOL, // guns  - bryar pistol slot, gun slots . 
	RIFLE, // rifles, - blaster slot. -fireballs magic 
	HEAVYREPEATER, // repeaters, WP_REPEATER, - crioacquei 
	SHOOTGUN, // carabine, FLECHETTE, - litici 
	ROCKET, // rockets, rpg, Air bolts. 
	MUSKETEER, // tusken rifle slot  
	RAY, // particle weapons - disruptor. sun ray magic 
	GRENADE, // throwing bombs explosive WP_THERMAL 
	TRAP, // trap explosive weapons as laser traps 
	EXPLOSIVE, // as det pack 
	HEAVYGUN, // atstmain slot. scintore, aggregatore, metallo. as magicla 	
	// THESE SLOT ARE SPECIAL. USED FOR MAGICAL CREATURES THAT SHOOT ELEMENTAL STUFF
	// AND ALSO FOR FIGHTERS WEAPONS FOR DOGFIGHTS 
	PSIONIC, // sonic \ stun \ psychic weapons jawa slot. 
	DARKNESS, // darkness elemental weapons - blaster pistol slot
	ENERGEN, // sun bolt weapons stuff  bowcaster slot 
	TERMOPIREUS, // fire bolt magic - termopiree, magical fires  blaster slot 
	IDROFROST, // water, slime, crystal, ice  repeater slot 
	LITHIC, // earth weapon flechette slot 
	HOLYNESS, // life, nature, lifer elements  demp2 slot 
	NECRO, // necromantic weapons - Concussion slot NUONIC weapons CONCUSSION SLOT 
	ARCANE,	// Special Arcane weapons and bolts - rapid fire conc disruptor slot 
	ALCHEMIC, // alchemical weapons- WP_CANNON,  
	ORGANIC, // organic shoot of animals NOGHRI STICK SLOT  
	LUNAR, // the more exotical weapons with the more strange effects.  Bloodgun slot
	STORM, // Storm matter weapons - Warning, UNPREDICTABLE EFFECTS XD TIE FIGHTER SLOT 
	DIVINE, // hyperdevastating weapons as WP_ATST_SIDE. NOT USED BY PLAYER. 
	WPCLASS_MAX
} WPClass_t;

typedef struct 
{
	char		*gunname;						//entry in .shoot if any
	char		*fullgunName;					//the "Proper Name" of the pistol, shown in the UI
	char		*WpnType;						//kind of Weapon (WP_BLASTER, WP_REPEATER, etc,
	// looking off into a *WPN file, if found, override all stuff with his stuff! 
	char		*ItmType;	// Item for dropping and world model, into ITM files of JKG. 
	char		*AmmoType; // Ammo type for the weapon from AMMO stuff of JKG... 
	char		*gunmodel;						//gun model
	char		*gunskin;					//gun custom skin
	WPClass_t	guntype;
}
gunInfo_t; 
...
// External Weapons stuff! as WPN FILES! DEFINITIONS! 
typedef struct
{
	// QQQ INIT ALL FORCE AND SOUND CUSTOM STUFF 
	int			ForceRagesound;					//game soundindex for turning on sound
	int			ForceRageLoop;					//game soundindex for hum/loop sound
	int			ForceRageOff;					//game soundindex for turning off sound
	int			ForceRageEffect;				//none - if set, plays this effect at the blade tag
	//***The following can be different for the extra blades - not setting them individually defaults them to the value for the whole saber (and first blade)***
	//= TENUTO PROVVISORIAMENTE COME REFERENCE =====================
	//done in cgame (client-side code)
	//int			trailStyle;					//0 - default (0) is normal, 1 is a motion blur and 2 is no trail at all (good for real-sword type mods)
	//char		g2MarksShader[MAX_QPATH];	//none - if set, the game will use this shader for marks on enemies instead of the default "gfx/damage/saberglowmark"
	//char		g2WeaponMarkShader[MAX_QPATH];	//none - if set, the game will ry to project this shader onto the weapon when it damages a person (good for a blood splatter on the weapon)
...
#define MAX_SABERS 2
#define MAX_GUN 2 // we can carry one gun for hand, not? :p 
...
typedef struct playerState_s {
	...
	//////// FOR CUSTOMIZE SOUND AND EFFECTS! 
	SpecialEffectSoundInfo_t SFXInfo; 
	...
	gunInfo_t	gun[MAX_GUN];
	saberInfo_t	saber[MAX_SABERS];
//	wpnInfo_t	wpn[MAX_WPN]; 
	qboolean	dualWeapons; 
	qboolean	dualSabers;
...
//	int			vehTurnaroundTime;
	...
	int			cloakFuel;
	// QQQ FOR CUSTOMIZE SHOOT WEAPONMODELS! 
	char		*ShootWeaponModel;
	int			gunEntityNum;// for recreate guns 
} playerState_t;
...
typedef struct entityState_s {// !!!!!!!!!!! LOADSAVE-affecting struct !!!!!!!!!!!!!
	...
	int		eFlags;
	int		eFlags2;		// JKG EF2_??? used much less frequently
	...
	qboolean	loopIsSoundset; //Added qtrue if the loopSound index is actually a soundset index
